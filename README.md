...
## Hybrid PDF invoices, and why they must never see a model

Step two of the extraction design, reshaped in review because "PDF" is
genuinely two things. A hybrid PDF (Factur-X in France, ZUGFeRD in
Germany — both real mandates) carries a complete EN 16931 XML invoice
as an embedded attachment. That XML *is* the authoritative data.
Sending it to a vision model would be a regression: substituting
inference for mandate-grade fact. So every PDF is checked for an
embedded invoice first, and when one is found it takes exactly the
path a directly-submitted UBL document already does — no model, no
confidence score, no loss.

Two real findings while building it. The PDF spec requires an EOL
before the `endstream` keyword, so the naive stream boundary corrupts
compressed data — harmless on a naive fixture, fatal on most
real-world Factur-X files; `/Length` is authoritative instead. And
filenames never appear literally: producers write
`(factur\055x\056xml)`, so a reader looking for `factur-x.xml` finds
nothing. Fixtures are genuine PDFs with real xref tables and real
compression, generated by `scripts/build-pdf-fixtures.py`. What to do
with an unreadable hybrid is per-channel configuration defaulting to
refuse. See `docs/decisions/0042-hybrid-pdf-invoices.md`.

## Vision extraction, and what cannot be verified from here

The third and last intake path, completing UBL/XML (exact), hybrid PDF
(exact), and photograph or scan (**inferred**). Reached only when
neither other path can apply — a PDF sent to the image endpoint is
refused outright, because a hybrid read as a picture would silently
substitute inferred data for mandate-grade data.

Stated plainly because it matters: the most important claim here is
untested. `env.AI` has no local simulation, so whether a real vision
model reads a real invoice well is unknown until someone runs it
against one. The image request shape is a well-supported inference
from Cloudflare's own provider changelog rather than the model's
documentation, isolated in one function so correcting it is a small
edit. Everything *around* the model is tested thoroughly — 38 tests
covering schema, prompt, coercion, refusals, and the PDF guard.

Decision 0041's type system does real work here, and caught a real
bug: the first coercion implementation stripped any leading
non-digits, turning `"approximately 500"` into a confident `500` —
