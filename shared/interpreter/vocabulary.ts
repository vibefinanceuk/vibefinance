...
export const DERIVED_FIELD_DESCRIPTIONS: Record<DerivedField, string> = {
  ...
  "po.line_matched":
    "true if this line's own amount and quantity are within the supplier's agreed tolerance of the purchase order line BT-132 says it answers. False, not absent, when BT-132 is missing or points at a purchase order line that does not exist, or the invoice's own BT-13 (purchase order reference) is absent entirely — a rule should be able to test 'this line could not be matched' directly rather than treat absence and a real mismatch as the same silence. A wholly non-PO invoice reads exactly the same as a real matching exception on this fact alone — test BT-13 is_present first if the two need telling apart.",
  "po.line_variance_pct":
  ...
  "po.line_reference_found":
    "true if BT-132 points at a purchase order line that actually exists. Splits 'nothing to compare against' from 'compared and disagreed' — po.line_matched is false in both cases, but a rule routing 'PO line not found' to a different team than a price or quantity dispute needs to tell them apart. False, never absent, so a rule can test it directly. **Also false, not absent, when the invoice carries no purchase order reference at all (BT-13 absent) — an ordinary non-PO invoice, not a matching exception.** A rule meaning only 'this invoice was supposed to match a PO and didn't' should test BT-13 is_present as well, or it will also catch every genuinely non-PO invoice.",
  "po.line_price_matched":
