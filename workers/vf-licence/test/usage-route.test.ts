...
describe("handleReportUsage — validation", () => {
  it("400s when a required field is missing", async () => {
    const result = await handleReportUsage(env.CONTROL_DB, { customerId: "acme" });
...
describe("handleReportUsage — recording", () => {
  it("records a usage report", async () => {
    ...
    const row = await env.CONTROL_DB.prepare(
      "SELECT invoices_processed, rules_evaluated, active_users, outcome_counts_json FROM usage_periods WHERE customer_id = ? AND period_key = ?"
    )
      .bind("acme", "2026-08")
      .first();
    expect(row).toEqual({
  ...
  it("is idempotent — a second push for the same period overwrites rather than duplicates", async () => {
    ...
    const count = await env.CONTROL_DB.prepare(
      "SELECT count(*) AS n FROM usage_periods WHERE customer_id = ? AND period_key = ?"
    )
      .bind("acme", "2026-08")
      .first();
    ...
    const row = await env.CONTROL_DB.prepare(
      "SELECT invoices_processed, rules_evaluated FROM usage_periods WHERE customer_id = ? AND period_key = ?"
    )
      .bind("acme", "2026-08")
      .first();
  ...
  it("keeps separate rows for different periods of the same customer", async () => {
    await handleReportUsage(env.CONTROL_DB, {
      customerId: "acme",
      periodKey: "2026-07",
      invoicesProcessed: 3,
    ...
    const count = await env.CONTROL_DB.prepare(
      "SELECT count(*) AS n FROM usage_periods WHERE customer_id = ?"
    )
      .bind("acme")
      .first();
  ...
  it("stores activeUsers as null when not provided, never fabricated", async () => {
    ...
    const row = await env.CONTROL_DB.prepare(
      "SELECT active_users FROM usage_periods WHERE customer_id = ? AND period_key = ?"
    )
      .bind("acme", "2026-08")
      .first();
