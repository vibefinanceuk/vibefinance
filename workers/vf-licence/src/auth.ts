 ...
 *   provisioning (POST /customers, POST /licences, key rotation).
 * - Per-customer auth (one random key per customer, shown once at
 *   creation, only its hash ever stored): protects the machine-to-
 *   machine calls each customer's vf-app instance makes (fetching its
 *   own licence token, pushing its own usage). Critically, this must
 *   prevent one customer's instance from acting as a different
 *   customer — a shared secret would not provide that.
 *
 ...
 */
export async function isValidCustomerKey(
  db: D1Database,
  customerId: string,
  ...
  const row = await db
    .prepare("SELECT api_key_hash FROM customers WHERE id = ?")
    .bind(customerId)
