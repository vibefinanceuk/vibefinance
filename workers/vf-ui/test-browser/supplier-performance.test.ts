...
/**
 * The Supplier Performance screen — decision 0416, the second real
...
const STRINGS = {
  locale: "en",
  strings: {
    "supplierperformance.heading": "Supplier Performance",
    "supplierperformance.sub": "How the supplier base is doing",
...
async function openSupplierPerformance(data: unknown, seen: string[] = []) {
  stubSpend(data, seen);
...
describe("the heading and the card the route returned", () => {
  it("reads Supplier Performance, with the card's own title and subtitle", async () => {
    await openSupplierPerformance({ currencies: [] });

    expect(document.querySelector(".topbar h2")?.textContent).toBe("Supplier Performance");
    expect(document.querySelector(".topbar .sub")?.textContent).toBe("How the supplier base is doing");
  ...
  it("asks the route for the chosen org, the same treatment every other analysis screen gives it", async () => {
    const seen: string[] = [];
    await openSupplierPerformance({ currencies: [] }, seen);
...
describe("a single currency reads as the plain simple list the design asked for", () => {
  ...
  it("draws one row per supplier, with no currency label wrapper", async () => {
    await openSupplierPerformance(DATA);
