...
/**
 * **Standard matching rules — decision 0474.** `load()` gained a 4th
 * required fetch; this file's own separate `openApSetupAs` (and every
 * inline `vi.stubGlobal("fetch", ...)` block below) needs the same
 * stub `ap-setup.test.ts` already carries, or every test in this file
 * fails the same way `/api/matching-config` itself once did.
 */
const EMPTY_STANDARD_RULES = { standardRules: [] };
  ...
  it("re-fetches with the search term once typing is done, and resets to page 1", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        ...
        if (path === "/api/matching-config/standard-rules") return { ok: true, json: async () => EMPTY_STANDARD_RULES } as Response;
