 ...
 *
 * **An eighth addendum, from a production smoke test after the sixth
 * and seventh addenda went live.** "Who is the most active AP team
 * member?" answered "there are currently no open tasks assigned to any
 * team member" on a workflow whose only open task was unclaimed rather
 * than owned — technically consistent with `tasks_by_user`'s own real
 * data (`openTasksPerPerson` genuinely was empty), but silently dropped
 * the one number that would have told the real story: `unclaimedAndAvailable`,
 * already present in that same tool's result, sitting right beside the
 * empty list. Every other tool's own similarly easy-to-miss field —
 * `documentUrl`'s presence-vs-absence, `moreMayExist`,
 * `unconfirmedAmountCount` — already gets its own explicit instruction
 * in `buildAnswerPrompt` for exactly this reason; `unclaimedAndAvailable`
 * never had. Fixed the same way as the rest: a new sentence telling the
 * phrasing model to mention it whenever `openTasksPerPerson` is empty
 * but it is not zero, rather than only reporting the per-person list
 * and leaving a real, available-to-claim number unsaid.
 */
