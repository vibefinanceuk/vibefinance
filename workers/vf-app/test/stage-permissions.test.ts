...
describe("the closed set, in two places", () => {
   ...
   *
   * **Seven migrations now, decisions 0350, 0417, 0430, and 0468.**
   * `0048`'s own standing invariant is re-checked forever and was never
   * edited in place; `0062`, `0063`, `0066`, `0071`, `0074`, and `0078`
   * each restate the same invariant with the vocabulary as it stood
   * when written, adding `Admin.RuleActivation`, `Admin.RoleManagement`,
   * `Supplier.Maintain`, `AP.FraudReview`, `AP.Assistant`, and finally
   * `Procurement.Collaborate`/`Procurement.Approve` respectively —
   * `AP.FraudReview` was still reserved when `0071` was written and has
   * had a real route since decision 0420; `AP.Assistant` gets its first
   * real route in the same decision (0430) that added it here, so it is
   * never "reserved" at all in this file's history; the two Procurement
   * permissions `0078` adds are, like `AP.Match` before them, reserved
   * from the start — no route accepts either yet. Combining every
   * file's own permission strings is what "the closed set, in two
   * places" now means — each already-applied migration's own list
  ...
  it("names exactly the permissions the code defines", () => {
    const inSql = [
      ...
      ...matchingExceptionsAndBusinessUserSql.matchAll(/'([A-Za-z]+\.[A-Za-z]+)'/g),
    ]
      .map((m) => m[1])
      .sort();
