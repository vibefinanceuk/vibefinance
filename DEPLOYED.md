...
|---|---|---|
| vf-app | `a79b783` | 4 September 2026 — the retention benchmark (decision 0077), migration 0032 applied. `GET /settings/retention` returns seven years and `enforcement: "none"` in the payload, because an operator reading a retention period could reasonably assume it was enforced. The report was exercised properly rather than accepted on a zero: with every invoice captured this year, an empty result is both the correct answer and the answer a broken query would give, so one invoice was temporarily backdated to 2015 and the report correctly returned it with `anchorSource: "issue_date"` — and happened to surface the `.bin` document from decision 0069's content-type bug, which is a fair demonstration of what the report is for. The backdating was then undone. Earlier the same day: `require_second_approval` removed (0074), returning built and exercised live (0075), and the migration checksum made to mean something (0076).
