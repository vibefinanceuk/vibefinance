...
| vf-licence | `602e854` | 4 September 2026 — version `3985c42a`. Credentials and access grants (0092, migration 0010), and the composite foreign keys that make a cross-customer grant impossible (0093, migration 0011). **Verified against the exact row that had reached production**: a hand-written `INSERT` granting `nobody@example.com` access to `Acme-production` succeeded earlier the same day and sat in the table until it was deleted — the row decision 0092 had claimed could not be written. The same insert is now refused with `FOREIGN KEY constraint failed`, and both keys are visible in the table definition, which was the argument for carrying `customer_id` rather than using a trigger. Also carrying login attempts (0090, migration 0009), password hashing (0089), session tokens (0086) and the dev login stub (0087) with `ALLOW_DEV_LOGIN` unset so `/dev-login` returns 404 here.
...
| vf-licence-poc | 0011_grants_carry_their_customer.sql | 2 September 2026 — applied through 0007; `warned_at_days` confirmed recording which threshold fired, verified live against `northwind-sandbox` and then restored. Recorded from the previous session's confirmation rather than re-verified today |
