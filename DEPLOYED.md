...
|---|---|---|
| vf-app | `99e2804` | 3 September 2026 — three corrections (decisions 0065, 0066, 0067). Settings propagation confirmed by direct query against `ap-live`: all four channels read `require_line_description=1, max_extracted_lines=25, currency_tolerance_minor=1, conflict_winner='first'`. **The migration was a no-op on this database**, because `ic-new` had never been configured away from the defaults — the correct outcome, and an undramatic one. What it confirms is that nothing was clobbered and the guard held; what it does not confirm is the copy itself, which was verified locally against seeded data instead, since the replay runs on an empty database where it would have been vacuously verified. The earlier `ae0ea7e` deploy (source-addressed capture, decision 0063) remains confirmed live — see the incident record for the Morrison run.
