...
|---|---|---|
| vf-app | `f148491` | 4 September 2026 — returning a document (decision 0075), version `e6982a1d`. Migration 0031 applied, adding the task state model (open / completed / returned / cancelled) and the terminal instance states. Decision 0075 also widened a standing invariant in **migration 0009**, which was refreshed with `--refresh-checksums`; the stored value now matches the file's `c5272738a170...` exactly, verified by direct query after the refresh reported drift. That round trip mattered: decision 0076, built the same day, found the checksum had been written on every apply and compared to nothing since the mechanism was written — under a comment asserting it was verified. **Return exercised live** the same day: task `186b4a17-...` at `approval`, claimed by alice and returned to `received` with a reason. The record afterwards reads `status: returned`, `ended_by: alice`, **`completed_by: null`** — which is the distinction the whole state model exists for: Alice declined the work and said why, and recording her as the completer would have put a lie in the audit trail. A fresh open task appeared at `received`, and the instance moved back while staying `in_progress`, because it genuinely is in progress, just backwards. `viaOverride: false` confirmed the ordinary path rather than a manager override. Not yet exercised live: return-to-supplier, and the `AP.ReturnAny` override.
| vf-licence | `f203ac0` | 2 September 2026 — signup requests and staged expiry warnings (decisions 0039, 0040) confirmed live: a real signup request provisioned Northwind's sandbox with a 30-day trial, and the 7-day expiry warning was confirmed firing against it and then restored. Recorded here from the previous session's own confirmation rather than re-verified today; nothing has touched `workers/vf-licence/src` since. |

## D1

| database | last migration applied | confirmed at |
|---|---|---|
| vf-app-poc | 0031_task_states_and_returns.sql | 3 September 2026 — applied through 0029. `sources` holds one row backfilled from `intake_channels` under the same id (`ic-new`, `https`, `legacy_channel_id` matching), so stored `mandate.channel` values keep resolving. `intake_channels` holds four rows for `ap-live`: the legacy row at `structure = NULL`, plus `ap-live-xml`, `ap-live-pdfa` and `ap-live-image`. All four confirmed carrying identical extraction settings after 0029 — a no-op here, since `ic-new` was never configured away from the defaults. The seeding in 0028 and the copy in 0029 were additionally exercised locally against seeded data, because the migration replay uses an empty database where both would have been vacuously verified.
| vf-licence-poc | 0007_licence_expiry_warnings.sql | 2 September 2026 — applied through 0007; `warned_at_days` confirmed recording which threshold fired, verified live against `northwind-sandbox` and then restored. Recorded from the previous session's confirmation rather than re-verified today |
