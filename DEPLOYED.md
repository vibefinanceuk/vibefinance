...
| vf-ui | `112ed13` | 4 September 2026 — the shared interface, at `https://vf-ui.vibefinance.workers.dev` (decision 0099). **The first thing in this project exercised in a browser rather than through `curl`**, and it worked: a person signs in, the environment list populates from what they may actually reach, Acme's livery arrives from `vf-licence` and turns the bar and button blue, and a session token comes back scoped to `Acme-production` with the attempts-since-last-sign-in report ISO 27001 A.8.5 asks for. `ALLOWED_ORIGINS` set on both API Workers to this origin, which is what lets a browser read either response (decision 0098) — until then the CORS machinery was built and inactive. Two deploys failed first on a missing trailing comma in `wrangler.jsonc`, which wrangler reports as `CommaExpected` pointing at the **new** line rather than the one missing the comma. |
