...
|---|---|---|
| vf-app | `79bbb74` | 4 September 2026 — purchase orders (decision 0081), migration 0034, version `c98c0f97`. Ingestion confirmed live: a Peppol BIS Order Only document stored with its line, then **re-posted and correctly replaced** — same id reused, `replaced: true`, still one order and one line, which is the guarantee that stops matching silently picking between two versions. Read-back showed totals taken from `AnticipatedMonetaryTotal` rather than `LegalMonetaryTotal`, and `unit_code: "NAR"` travelling with its quantity. Nulls where the document omitted optional fields, which is the parser honouring a spec that permits their absence rather than a gap. The order uses Morrison's VAT id and a 3,137.47 total deliberately, so there is a real order for that invoice to match against once matching exists. Also deployed, and not a migration: `ap-live`'s process definition corrected from three stages to seven (decision 0080).
| vf-licence | `f771d7e` | 4 September 2026 — environment deletion (decision 0085), and earlier the same day one production per region (0084, migration 0008). **Both verified in both directions against production**, which mattered because two earlier versions of 0008 were wrong. For 0084: creating `Acme-production-us` succeeded and a second production in `eu` was refused naming the region; the schema was checked directly, with `licences`, `usage_periods` and `signup_requests` all still reading `REFERENCES environments(id)` — precisely what the rename-aside attempt got silently wrong — and `licences.warned_at_days` surviving the hand-written recreate. For 0085: `Acme-production-us` was deleted, and `Acme-production` was **refused** with both blockers named (`1 licence(s), 2 usage period(s)`) rather than stopping at the first. Data intact throughout: 2 environments before the tidy-up, 2 licences, 2 usage periods, 1 signup request, 0 holding tables. The key issued for the temporary environment was rotated immediately, having been shown in a shared session.
...
| vf-licence-poc | 0008_environments_per_region.sql | 2 September 2026 — applied through 0007; `warned_at_days` confirmed recording which threshold fired, verified live against `northwind-sandbox` and then restored. Recorded from the previous session's confirmation rather than re-verified today |
