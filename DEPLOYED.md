...
|---|---|---|
| vf-app | `ae0ea7e` | 3 September 2026 — source-addressed capture (decision 0063) confirmed live. `POST /sources/ic-new/capture` given the real two-page Morrison PDF: detection found a PDF header, found no embedded invoice, and captured the document with provenance and no facts rather than rejecting it. A real instance was created, reached `approval`, and rule `96abd97c` (*"if validation has not passed, assign a task"*) fired and raised a task — a document nothing could read reaching a person through a rule written in plain English. `facts_json` confirmed by direct query as `{"intake.structure":"","intake.attempted":"pdf_header,embedded_invoice_xml"}`: the empty string rather than an absent field, which is what makes it testable by a rule. Commits after this one are documentation only and change no Worker code.
...
|---|---|---|
| vf-app-poc | 0028_intake_channel_structure.sql | 3 September 2026 — sources and structural channels confirmed live by direct query. `sources` holds one row backfilled from `intake_channels` under the **same id** (`ic-new`, mechanism `https`, `legacy_channel_id` matching), so every stored `mandate.channel` value keeps resolving with no data migration. `intake_channels` holds four rows for `ap-live`: the legacy row at `structure = NULL`, plus `ap-live-xml`, `ap-live-pdfa` and `ap-live-image`. The seeding SQL was additionally run against seeded data locally and the rows inspected, because the migration replay uses an empty database where it would have been vacuously verified.
