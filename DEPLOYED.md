...
| vf-licence | `f5a0cd0` | 4 September 2026 — session tokens (0086) and the development login stub (0087), version `87c65437`. **`ALLOW_DEV_LOGIN` is deliberately unset here**, so `POST /dev-login` returns 404 on the Worker serving real data — confirmed directly. A 404 rather than 403 because a 403 would tell somebody probing that there is a route to enable. Even were the flag set, the stub refuses to mint a token for a `production` environment, which is the guard that cannot be forgotten. Earlier the same day: environment deletion (0085) and one production per region (0084, migration 0008), both verified in both directions — creating `Acme-production-us` succeeded and a second in `eu` was refused; deleting that temporary environment succeeded and deleting `Acme-production` was refused with both blockers named. The schema was inspected after 0008's four-table rebuild rather than inferred from the data surviving.
