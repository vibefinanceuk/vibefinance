...
|---|---|---|
| vf-app | `fe4a216` | 3 September 2026 — keying (decision 0071) confirmed live, closing the loop the Morrison document opened this morning. Invoice `3e0469f8-...` had been captured with **no invoice facts at all** — detection found a PDF header, no embedded invoice, and it was retained rather than rejected. `POST /invoices/:id/key` then recorded `BT-1`, `BT-112` and `BT-5` with `keyedBy: alice` derived from the token, `corrected: false` on each since extraction never produced them, and `provenance.keyed` listing all three. **The spoofed-identity guarantee was verified against production, not just in tests**: a second call carrying `"keyedBy":"someone-else"` in the body still recorded alice, and `provenance.keyed` accumulated to four fields rather than being replaced by the one just keyed. Earlier the same day: retention (0068), its content-type correction (0069) confirmed by `application/pdf` under a `.pdf` key, and the response reporting what it stored (0070).
