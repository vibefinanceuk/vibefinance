...
|---|---|---|
| vf-app | `ec0c60a`+ | 5 September 2026 — the **task listing** and its actions (decisions 0103, 0104, 0105). `GET /tasks` returns a person's own work across every stage with ownership as a column, filtered by stage and ownership, each task reporting what the server will honour. **Claiming worked from `curl` and failed from the screen**: the route used `requirePermission`, which understands API keys only, and predated sessions. No test caught it — `authenticateUserOrSession` was tested and worked, while nothing tested *which routes call it*, so 927 tests passed with the button doing nothing. Fixed, and closed with a test keypair in `wrangler.test.jsonc` that lets the router itself be exercised. **Confirmed live: a person can now claim a task from the Task Manager.**
