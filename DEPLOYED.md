...
| vf-licence | `98dac9b` | 4 September 2026 — version `d3357b00`, carrying password hashing (0089) though nothing calls it yet. **The first deploy failed**: `@noble/hashes` had been installed at the repository root rather than into `shared`, and wrangler could not resolve it — even though `vf-licence` does not use password hashing at all, because `shared/index.ts` re-exports everything and the barrel's dependencies must all resolve. Fixed and verified with `--dry-run` on both Workers before handing over. The bundle is **49.88 KiB, unchanged**, since esbuild tree-shakes the unused code away: the barrel costs nothing at runtime and everything at build time. Also carrying session tokens (0086) and the development login stub (0087), with `ALLOW_DEV_LOGIN` deliberately unset so `POST /dev-login` returns 404 here. Earlier the same day: environment deletion (0085) and one production per region (0084, migration 0008), both verified in both directions.
