...
|---|---|---|
| vf-app | `b7df20c` | 3 September 2026 — the re-visit guard and the post-keying validation report (decision 0072), both confirmed live. **The guard was verified against the exact hazard it prevents**: `POST /process-instances/c4f1a554-.../visit` on an instance waiting at `approval` returned 409 naming one open task, where before this deploy it would have raised a second identical task and nothing would have flagged it. Keying then reported `passed: true` with `vat_arithmetic` among `checked` — the first time keying has answered "is it valid now". Note `checked` held only two entries: `amount_due_mismatch` and `line_sum` could not run without `BT-115` and lines, which is decision 0044's separation of `checked` from `failures` doing its job rather than a gap. Earlier the same day: retention (0068), its content-type correction (0069), the response reporting what it stored (0070), and keying itself (0071) including its spoofed-identity guarantee.
