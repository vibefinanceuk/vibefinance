...
|---|---|---|
| vf-app | `8b8290f`+ | 5 September 2026 — **which part of the enterprise an invoice belongs to** (decisions 0111, 0112), plus line keying (0109) and the BG-25 vocabulary (0110). `org_units` is load-bearing for the first time since decision 0003 created it. **All three placement paths proven live**: a rule using `assign_org`, a source default, and `PUT /invoices/:id/org` by hand — which produced the first ever `org_assigned_by = 'manual'`, a value that had been in the schema's `CHECK` and unreachable. The guard holds both ways: an operating unit cannot sit under another operating unit, and an invoice cannot be assigned to a legal entity, *"which is not where payables happen"*. Twelve mandatory EN 16931 terms added across two audits — including BT-27, the seller name, whose absence is why every screen showed a VAT identifier where a person expects a company.
| vf-licence | `cf8adef` | 5 September 2026 — **the interface's words** (decision 0107, migrations 0013 and 0014) and **branding** (0096, migration 0012). Every visible string in `vf-ui` now comes from D1 rather than the JavaScript, so fixing a wording or adding a language needs no deployment — the operator's requirement. 49 keys in English and German, with the other four of the six supported languages waiting on rows and nothing else. `GET /ui-strings/keys` gives a translator every key with its gaps and coverage per language; `POST /ui-strings` takes a whole language at once, all-or-nothing. The locale comes from the browser rather than a deployment var, because decision 0008's reasoning — one Worker per customer, one language — does not carry to a UI that is one shared deployment. Also carrying credentials, access grants and login (0092, 0094).

| vf-ui | `d7d7dbe` | 5 September 2026 — the **Task Manager**, the **Validation viewer** and the **interface direction** (decisions 0103, 0106, 0108). A person signs in, sees their work across every stage with ownership as a column, claims a task and keys the document behind it. **Reworked against a stated visual direction** rather than assembled by instinct: a persistent frame, panels rather than one column, status first and separately, actions collected in one place, and colour carrying meaning and nothing else. Every visible word comes from D1 (0107) and every colour from a token (0096). The layout reflows rather than shrinks below 1100px — the constraint a mockup rendered at 1536 never shows. Earlier the same day: refreshing briefly showed the sign-in form before the list, because both views were not hidden while the page decided which to render.
