...
| vf-ui | `ec0c60a` | 5 September 2026 — the **Task Manager** (decisions 0103, 0104). A person signs in and lands on their own work: one list across every stage, ownership as a column, buttons drawn from what each task says the server will honour, and filters by stage and ownership. **Refreshing holds steady on the list** — the session has survived a refresh since the backend-for-frontend landed (0102), and nothing was asking, so a reload showed an empty sign-in form while the session was perfectly alive. A first fix left the form visible by default, which made it paint and vanish on every refresh; both views now start hidden, so nothing flashes. Reported from the browser rather than caught by a test: nothing was broken, and what was wrong was what the page said **while it was deciding**.
