...
|---|---|---|
| vf-app | `ff583d3` | 3 September 2026 — signed document URLs (decision 0073) confirmed live, completing the chain the Morrison document opened this morning. `POST /invoices/:id/document-url` returned a five-minute link and `contentType: application/pdf`; opening that link in a browser with **no Authorization header** displayed the retained PDF inline. `DOCUMENT_URL_SECRET` set via `wrangler secret put`, generated by the operator and never passing through a shared session — the practice decision 0012 settled after the private-key incident. Earlier the same day, all confirmed live: retention (0068), its content-type correction (0069), the response reporting what it stored (0070), keying with its spoofed-identity guarantee (0071), and the re-visit guard (0072) verified against the exact duplicate-task hazard it prevents.
