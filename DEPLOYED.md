...
| vf-licence | `8240c29` | 4 September 2026 — one production per region (decision 0084), migration 0008, version `29895bb7`. **Verified in both directions against production**, which mattered because two earlier versions of this migration were wrong: creating `Acme-production-us` succeeded, and a second production in `eu` was refused naming the region. The schema was checked directly too — `licences`, `usage_periods` and `signup_requests` all still read `REFERENCES environments(id)`, which is precisely what the second attempt got silently wrong, and `licences.warned_at_days` survived the hand-written recreate. Data intact: 2 environments, 2 licences, 2 usage periods, 1 signup request, 0 holding tables. The key issued for the new environment was rotated immediately, having been shown in a shared session.
