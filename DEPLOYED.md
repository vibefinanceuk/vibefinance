...
|---|---|---|
| vf-app | `793f439` | 4 September 2026 — discarding (decision 0078), migration 0033. That migration **rebuilt the `tasks` table**, since SQLite cannot alter a CHECK in place; verified afterwards by direct query that every column, both foreign keys and the constraint survived on live data, including a returned task's reason text and a line-level task's `line_number`. Separately and not a migration: `ap-live`'s process definition was corrected from three stages to seven (decision 0080) via `docs/operations/`, moving six validation rules off Approval. A fresh capture then stopped at **Validation** rather than Approval, which is where an unreadable document belongs.
