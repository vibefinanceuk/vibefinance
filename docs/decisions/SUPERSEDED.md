...
| **0209** — every supplier has an ERP identifier | 0231 | `NOT NULL` on an argument about **payment**, enforced as a rule about **existence**. A supplier may be recorded before the ERP has one — which is how a new supplier ordinarily arrives, and refusing it forced the work onto paper. |
| **0213** — there is no *Add supplier* | 0231 | Still true of a supplier the ERP **has**. What can be recorded here is one it **does not**, which is telling the master what is missing rather than overriding it. |
| **0329** — a popout's own close icon, relocated | 0330 | Moved only the icon that was already one, leaving Save/Create/Assign as a separate, bottom-of-form button. The operator's own follow-up wanted that action beside Close, both top right — the shape decision 0306 already gave the supplier popout, checked for and missed the first time. |
| **0280** — the address, stacked under its own label | 0387 | Right for a column half the card's own width — stacking gave the address its own full half rather than sharing it with an 88px label. `.sellergrid` is one full-width column now, so the same trade no longer holds, reported live once the wider card was in front of the operator: "now the address can sit to the right of the address title, rather than beneath it." |
| **0218** — the ERP identifier is not editable | 0231 | Holds for a row the ERP owns. **Filling in a blank is not overwriting a value**, and it is how a supplier awaiting the ERP stops awaiting it. |
| **0219** — a separate card, and `0224`'s sub-line | 0220, 0227 | Space, and then a name said twice once decision 0226 made the entity and the unit one row. |
| **0372** — "unlike Suppliers there is no Change, no hold, no status" | 0377 | True when first built, and never revisited until the operator asked for the same status ring Suppliers already had. Active/On-Hold/Closed is now a real, assignable column, with Hold/Release Hold/Close mirroring Suppliers' own mechanism (decision 0230). |
| **0213** — one customer's supplier master fits in memory at once, so `handleListSuppliers` returns everything and the status ring counts a fully-loaded array in the browser | 0378 | True until a customer's own list grew large enough that loading it whole stopped being viable, the same pressure decision 0376 already responded to for Purchase Orders. Real, server-side search, pagination, and a dedicated status-counts endpoint replaced both. |
| **0068** — the multi-page pending-document flow "deletes on finalise" | 0381 | Checked directly while scoping the document viewer's next piece of work, and found false: no `delete` on `PendingDocumentStorage`, no `.delete(` call anywhere, no `DELETE FROM pending_document_pages`. Every page is still in R2, and was the whole time — only unreachable, because `handleFinalisePendingDocument` never links it into `invoice_documents`. Phase 1 built the read path (`listRetainedPages`, `retainedPage`) rather than reversing a deletion that never happened. |
| **0123** — "a frame left open through a long keying session goes blank, and nothing refreshes it" | 0380 | Half right. **Nothing refreshed it** — while a later comment in `viewer.js` claimed something did, on returning to the tab. **It does not go blank either**: measured in Chromium, a frame whose link has expired keeps showing the document through scrolling, zooming, hiding and tab switches, and goes wrong only when it loads again — showing `vf-app`'s JSON error. The frame's own `load` event is the signal now, not a timer, which would have reloaded a working frame and lost somebody's place in it. |
| **0380** — the reload guard applied to "both frames... the Document tab's preview... and the XML tab" | 0382 | True when written. The Document tab has no frame to guard once decision 0382 replaced it with a canvas — pixels already drawn, so nothing reloads it and nothing can ask a stale link again. The guard is unchanged and still load-bearing for the XML tab's own, still-live `<iframe>`. |
| **0018** — a Factur-X/ZUGFeRD hybrid needs "one 'original' row only, since the received PDF/A-3 already embeds the XML — no separate rendering needed" | 0383 | True of extraction, and it undersold what a person wants to see. The embedded XML `pdf-attachment.ts` reads for the outer PDF was read once and thrown away, so a hybrid invoice had no XML tab at all, unlike a bare-XML invoice retaining the same kind of data. `document_type` gained a genuine third value, `'embedded_xml'`, retained alongside `'original'` rather than replacing it — the table's other invariants (the `UNIQUE` constraint, the two pre-existing cases) are untouched. |
| **0267** — the activity panel as a drawer beneath the preview | 0269 | The drawer stood for a day. The operator's own picture was two tabs, Document and Timeline / Chat, with the trade-off — never seeing both at once — confirmed first (0268). Everything 0267 built behind the drawer stands. |
| **0122** — a row of actions beneath the document, `expand` and `save` always in it | 0142, 0298 | Save stopped always appearing in 0142, where nothing is editable. Then the row itself went: Save and the task's actions to the topbar beside Back, Expand beside the document's own tabs — the operator's request, *"This will free space below the document image."* |
| **0175** — the heading labelled `Stage: Validation` | 0312 | Labelling the stage was right against a bare word. Naming it at all was a duplicate once the Process flow above the columns highlighted the same stage. The heading names the document — `Unique Ref:` — and a task outside any process shows no stage anywhere, by the operator's own choice. |
| **0105** — "`complete` is deliberately among [the API-key-only routes]… That button belongs on the viewer" | 0138 | It went on the viewer, and the route had to accept a session to be reached from there. After 0138, every route in `vf-app` accepts one. |
...
| **0334** — role assignment and properties, one combined pop-out | 0337 | Merging role assign/revoke with a new properties, approval-limit, and spend-limit editor into one pop-out was reported live as *"not very user friendly"* — split back into two, each reached by its own icon rather than a row click. Everything decision 0334 actually added (the properties themselves, both limit editors) stands unchanged; only the single-pop-out shape it chose is gone. |
| **0341** — `flex: 1` on the currency field | 0343 | Fixed the field being squeezed to a sliver by letting it grow to fill the row — correct for the row's own original shape (0332's team-member picker), wrong once nothing else in the currency row competed for space, reported live the very next message as *"now too wide."* A fixed width with a floor replaced it. |
...
| **0221** — the country expanded to its Peppol long name | 0389 | Reasoned at the time as *"say what the image says"* — and OpenPEPPOL's long form (`GB` → "United Kingdom of Great Britain and Northern Ireland") is not what an invoice actually prints either. Asked directly to revert: *"I think in all cases, we can stick with the short form country code."* Everything else 0221 built — inline labels, the address block, the phone number — is untouched; only the country line changed. |
| **0390** — `.vpreview`'s leftover `min-height: 320px` "still the floor for a card genuinely given less than that" | 0391 | True only until the Document card was actually capped to less than 320px on some invoices (the very next request) — at which point that floor did exactly what a floor does, growing `.vpreview` past its own container's bottom and spilling into the Lines panel below it. `min-height: 0` replaced it for the docked case; the base rule's `320px` still stands for the pop-out window, which was never in question. |
| **0333** — `.tabbar`'s own underline-only shape, "no pill background, no card wrapper, so it reads as navigation within the page" | 0399 | True as a description of what was built, and asked to change directly: "the most recent tab select update to pill box, also... applied to the tabs in the Access screen." `.tabbar`/`.tab`/`.tab.active` now share `.doctabs`'s own pill shape and its `--tab-active-shadow` token, under the same class names. Everything else 0333 built — the rename, the tab-visibility gating, teams' own org — is untouched. |
| **0398** — `.doctabs` "a separate component from `.tabbar`/`.tab`... the two keep different shapes on purpose" | 0399 | Right when written, for the reason given — Document/XML/Timeline & Chat read as views of one thing where Access's own tabs read as sections of a page. The operator's own next request asked for visual consistency across both anyway, superseding the distinction rather than finding a flaw in it. `.doctabs`'s own shape and shadow token are untouched; only the case for keeping `.tabbar` apart from them no longer holds. |
| **0055** — section 7, "**The system does not send anything back to the sender**" | 0498 | True until the operator asked for a real, working communication path on the Return To Supplier button — reasons, a comment, and (asked directly) actual delivery to the supplier's own email, with an optional AP-team CC. Resend now sends it, with delivery/bounce tracking via its webhook. The rest of section 7 is untouched: the two terminal states (`returned_manually` / `archived`) and the case for keeping them as instance status rather than a new stage both stand exactly as written, and 0498 answered the operator's own "do I need an Archive stage?" question by pointing back to them. |
...
| **0231** | Matched, or payable? | **Two claims.** `supplier.matched` says we recognise them; `supplier.awaitingErp` says the ERP cannot name them yet. Decision 0209's argument survives as a field rather than a constraint. |
...
| **0124** | Which font? | **0124** — Carlito, metric-compatible with Calibri and openly licensed. *(Asked live and answered in the same record. This row named 0113, which is about code lists and never mentions a font — corrected in 0380.)* |
