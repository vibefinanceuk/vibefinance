...
23 September (thirty-five times), updated 24 September (twenty-eight times).**
...
| | |
| --- | --- |
| `origin/main` | **Decision 0477 ("PO line not found" vs. an ordinary non-PO invoice — reported live after the operator actually authored decision 0474's own "Standard rule: PO line not found": *"If a non-po invoice is received with no PO on it, will the item be able to bypass the matching queue with these rules in place. I expect that Non-PO invoice will stop in the Coding queue."* Traced through the code: no, as the suggested sentence was written, a wholly non-PO invoice would have been caught by it too — `po.line_reference_found`/`po.line_matched` read `false` for both a real matching exception and an ordinary non-PO invoice with no `BT-13` at all, and `assign_task` blocks stage progression. The other three standard rules don't share this bug — their own facts are left genuinely *absent*, not `false`, when there's no PO line to compare against, and an absent fact never fires an `is`/`is_not` condition. Fixed at the suggested-sentence level, not with a new fact — `BT-13` is already a real, `is_present`-testable vocabulary field, so `STANDARD_MATCHING_RULES`'s own `po_line_not_found` suggested sentence now reads *"If the invoice has a purchase order reference and a purchase order line cannot be found..."* `DERIVED_FIELD_DESCRIPTIONS` for both facts corrected too, since that text is what the compiler's own model is shown, not only documentation. **Not retroactive — the operator's own already-compiled rule, built from the old sentence, keeps the ambiguity until re-authored by hand.**) is confirmed pushed and deployed — `origin/main` fetched directly reads `9b0a134`, matching this session's own commit exactly, and the operator confirmed with *"this is pushed and deployed - I have updated the rules with a new version."*** `shared` interpreter/compiler suites **159/159**; `vf-app` `matching-config-route.test.ts` **15/15**, `po-matching.test.ts` + `rules-list.test.ts` + `rule-set-loader.test.ts` together **75/75**. `eslint` clean on both touched files; `tsc --noEmit` shows no new errors. Two pre-existing, unrelated `shared` test failures (`licensing/token.test.ts`, `migration/table-classes.test.ts`) confirmed present on unmodified `origin/main` via `git stash`. No migration — `shared`/`vf-app` only, no `vf-ui`/`vf-licence` change. The operator has already re-authored their own "Standard rule: PO line not found" with the corrected sentence — nothing further to apply. Full reasoning in decision 0477.** **Decision 0476 (removing a collaborator, restricted to a new `AP.Manager` permission — `AP.Manager` deliberately narrower than the `AP.Review` that adds one, per the operator's own instruction: *"Yes, but I'd like to restric[t] the ability to remove a collaborator to the AP.Manager role."* Investigated first, not assumed: "AP Manager" is not a real permission or seeded role anywhere in this codebase, only ever an illustrative role name in docs/tests — put to the operator directly, answered "new permission." New `handleRemoveCollaborator` (`invoice-collaborators-route.ts`), 404-on-no-op matching `handleRemoveTeamMember`'s own precedent, not `handleAddCollaborator`'s no-op-not-409; new `DELETE /documents/:id/collaborators/:userId`; a small "x" on every chip in `collaborators.js`, shown unconditionally per this file's own "never hide a control the caller cannot use" habit, no confirmation dialog. Two real, pre-existing gaps found and fixed in the same pass, neither this decision's own scope: `GET`/`POST /documents/:id/collaborators` — decision 0470's own routes — had never been added to the `vf-ui` proxy allowlist at all, the exact recurring class of gap decisions 0418–0431, 0441, 0473, and 0474 keep finding; and `collaborators.js` itself carried zero dedicated browser-test coverage since 0470 shipped — four new tests added, scoped to this decision's own removal behaviour only.) is confirmed pushed and deployed — `origin/main` fetched directly reads `fb9fe55`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed, migrations applied"* — both in one report, including migration `0162`'s own separate apply.** `vf-app` test/index.test.ts + invoice-collaborators-route.test.ts together **194/194**; `vf-licence` full suite **320/320**, string-coverage **10/10**; `vf-ui` default suite **74/74**, `viewer.test.ts` (browser) **181/181**, full `npm test` **1080/1081** — the one failure, `typography.test.ts`'s own pre-existing hardcoded `font-size: 10px` on `.collabchip .activityavatar`, confirmed present on unmodified `origin/main` via `git stash`, unrelated to this decision. `eslint` clean on every touched file across all three workers. No `vf-app` migration — `invoice_collaborators` already existed and was already live from decision 0470. Full reasoning and verification counts in decision 0476.** **Decision 0475 (AP.Match's own route-widening — the identical gap decision 0456 already fixed once for AP.Code: `AP.Match` was real in the closed permission vocabulary and written into every one of decision 0474's own `STANDARD_MATCHING_RULES` `assign_task` actions, but checked by zero routes, so an `AP.Match`-only holder could claim a matching task and then find every next step forbidden. Five routes widened — `GET /invoices/:id`, `POST /invoices/:id/document-url`, `GET /invoices/:id/pages`, `POST /invoices/:id/pages/:n/document-url`, `GET /invoices/:id/progress` — the identical five decision 0456 widened for `AP.Code`. Deliberately narrower than `AP.Code`'s own widening, checked directly rather than copied blindly: `POST /invoices/:id/key`, the coding-list search routes, and `GET /tasks` all left untouched — decision 0467 already found resolving a matching exception never writes keyed facts through `/key`, Matching never searches the coding lists, and `AP.Match` shares `AP.Code`'s own operational assumption that a role granting task-bearing AP work grants `AP.TaskView` alongside it.) is confirmed pushed and deployed — `origin/main` fetched directly reads `2fea81a`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed."* `vf-app` only — no `vf-ui`/`vf-licence` change, no migration, so nothing further to apply. `test/index.test.ts` **173/173** (9 new), run together with eight other permission/task/coding test files — **372/372**, zero regressions; `test/scoped-roles.test.ts` **48/48**. Full reasoning in decision 0475.** **Decision 0474 (Standard matching rules — checkboxes on AP Setup's own Matching tab that only enable/disable a rule that already exists, reusing `PUT /rules/:id/enabled` (decision 0155) directly; settled by the operator's own free-form answer, narrower than either option offered: "Check box only used for activating / deactivating existing compiled rules" — no `/rules/compile`, `/rules/examples/:id/confirm`, or activate route reachable from this feature at all. New `GET /matching-config/standard-rules`, a closed lookup over exactly four canonical `rules.name` values, never a broader "every rule" fallback. New migration `0161`, six `ui_strings` keys, en/de.) is confirmed pushed and deployed — `origin/main` fetched directly reads `d1e2432`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed, migrations applied"* — both in one report, including migration `0161`'s own separate apply. This session followed decision 0473's own lesson this time: the new route's proxy-allowlist entry and its `CALLED_BY_A_SCREEN` test landed in the same pass as the route itself, and the full `vf-ui` `npm test` script ran (74/74 plain, 1076/1077 browser — the one failure, `document-window.test.ts`, confirmed pre-existing on a clean `origin/main` tree via `git stash`, unrelated). No `vf-app` migration needed — `AP.Match`'s own route-widening and removing a collaborator remain separately named, untouched here. Full reasoning in decision 0474.** **Decision 0473 (Matching Tab: the proxy never carried its own new routes — the exact gap decision 0441 already fixed once for this same screen. `GET`/`PUT /matching-config` were real and tested in `vf-app`, never added to `vf-ui`'s own explicit proxy allowlist, `PROXIED_TO_INSTANCE` — reported live as *"pushed and deployed - however I get an error message accessing AP Setup - saying 'AP Setup could not be loaded'"*, the identical symptom decision 0441 already named. Not caught for the identical reason decision 0441 already diagnosed: this session ran `vf-ui`'s browser suite repeatedly but never the package's own full `npm test`, the exact habit 0441's own closing section asked future sessions to follow. A second, related gap found while fixing this one, not from a live report: `PUT /approval-config/cost-object-dimensions` (decision 0452) was also missing from the same allowlist.) is confirmed pushed and deployed — `origin/main` fetched directly reads `5dfe311`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed."* `vf-ui`-only fix, two new allowlist entries plus three new `CALLED_BY_A_SCREEN` test entries; nothing in `vf-app`/`vf-licence` changed, so no new migration — nothing further for the operator to apply for this one.** **Decision 0472 (the real AP Setup Matching tab — `matching-config-route.ts`'s new `GET`/`PUT /matching-config`, gated by `Admin.Configure` the same as every other AP Setup route; `ap-setup.js`'s Matching tab, previously a placeholder, replaced with a real form for `org_matching_config`'s org-wide default amount/quantity tolerance and quantity-matching toggle, percentages entered as plain numbers) is confirmed pushed and deployed — `origin/main` fetched directly reads `bfb9cf9`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed."* **Deploying it alone did not resolve the operator's own next report — see decision 0473 just above: the proxy allowlist gap, not this decision's own code, which was already correct.** A real regression this decision's own change caused was found and fixed before delivery, not left for the operator to hit: `coding-lists.test.ts` also opens `ap-setup.js` (to reach Account Coding) through its own separate stub setup, which had never needed to stub `/api/matching-config` before this decision made that fetch load-blocking the whole screen — caught by running the full `vf-ui` browser suite (32/32 → 1/32 in that file), fixed by adding the route to that file's own helper and its three hand-rolled inline stub blocks, confirmed back to 32/32; full suite afterward **1070/1071**, the one remaining failure (`typography.test.ts`) confirmed pre-existing against unmodified `HEAD` directly. **Migration `0160` (`vf-licence`, five new strings for the tab's own labels) is now applied and confirmed live too — the operator's own report, *"migrations have been applied."*** No `vf-app` migration this time — `org_matching_config` already existed and was already live from migration `0078` (decisions 0465/0468/0469); the tab is inert with today's exact defaults (`0`/`0`/on) until an operator changes a value deliberately.** **Decision 0471 (the Business Approver role — `Procurement.Approve` made real: `approval-hierarchy.ts`'s new async `resolveNonPoApprovers` routing every qualifying invoice collaborator in parallel instead of decision 0469's own single "requester," each task now carrying `required_permission: "Procurement.Approve"` via a new optional override on `ApprovalResolution` that `workflow-engine.ts`'s task-creation loop honours ahead of the stage's own default, `index.ts`'s `GET /tasks` widened to also accept `Procurement.Approve` alone with `ownership` forced to `"mine"`, and `viewer.js`'s existing Complete button relabelled "Approve" for exactly these tasks via one new `action.approve` string) is confirmed pushed and deployed — `origin/main` fetched directly reads `08115f4`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* Settled directly by the operator before any code was written: unanimous completion when routed to more than one Business Approver, every collaborator holding the role (not only the earliest-added), and entry via the existing Tasks list rather than a new screen. Two pre-existing `vf-ui` test bugs found and fixed along the way — see decision 0471 itself for detail, including a real but separate `viewer.test.ts`/`tasks.test.ts` fetch-stub gap flagged, not fixed. **Migration `0159` (`vf-licence`, the one new `action.approve` string) still needs its own separate apply — see the `vf-licence-poc` migrations row.** No `vf-app` migration this time — `invoice_collaborators`, `org_approval_config`, and `Procurement.Approve` all already existed and were already live from decision 0469.** **Decision 0470 ("Add person to conversation," Phase 2 — `invoice-collaborators-route.ts`'s new `handleAddCollaborator`/`handleListCollaborators`/`isInvoiceCollaborator`, five existing routes widened via a new `canViewInvoiceAsCollaborator` helper so a `Procurement.Collaborate` holder added to an invoice can actually open it, not only its chat, plus two new routes and `vf-ui`'s new `collaborators.js` roster bar) is confirmed pushed and deployed — `origin/main` fetched directly reads `7ca7527`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* No `vf-app` migration this time — `invoice_collaborators`/`Procurement.Collaborate` both already existed and were already live from decision 0469. Migration `0158` (`vf-licence`, the five new "Add person" strings) is now applied and confirmed live too — the operator's own report, *"I see the add person in the chat now,"* the roster bar showing its real label rather than a raw key.** **Decisions 0464 through 0469 (two-way matching-exceptions investigation and design, then Phase 1 built — migration `0078`, the four split `po.line_*` vocabulary facts, `po-matching.ts`'s org-wide default tolerance and quantity-matching toggle, `Procurement.Collaborate`/`Procurement.Approve`, and Non-PO Approval routing across `approval-hierarchy.ts`/`workflow-engine.ts`) are confirmed pushed and deployed — `origin/main` fetched directly reads `9bb54e2`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed,"* then separately *"yes - sql is deployed"* confirming migration `0078`'s own `apply_migrations.py --remote` run — see the `vf-app-poc` migrations row. The Non-PO Approval toggle stays inert regardless of its own setting, since no route yet populates `invoice_collaborators`.** **Decision 0463 (the Fraud Prevention Dashboard's own "Potential Duplicate Invoices" table, the Dashboard's `possible_duplicates` card, and the Documents screen's `duplicates=1` filter all shared the same `duplicate_confidence >= 0.5` bar — reported live as *"shows as Empty even though I'd sent in the exact same invoice many times, with an increment on the invoice number only,"* which scores exactly `0.4` (matching total `0.25` + matching date `0.15`, no invoice-number match) and never once cleared `0.5` on any of the three screens, proven by an existing test that already asserted that `0.4`. Put to the operator directly: lower the threshold (chosen, over reweighting the score itself, since `invoice.duplicate_confidence` is a real field a customer's own rule can reference with its own chosen threshold) and no backfill (chosen, since a lowered bar of `0.4` already clears the operator's own already-submitted test invoices' stored score with no rescore needed). New `POSSIBLE_DUPLICATE_THRESHOLD = 0.4` constant in `invoice-history.ts`, imported by all three consumers rather than each restating `0.5` — the identical single-source-of-truth lesson decisions 0461/0462 learned in CSS, applied here in SQL) is confirmed pushed and deployed — `origin/main` fetched directly reads `e6f4604`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* No migration — `vf-app` only, no new string key.** **Decision 0462 (the actual reason the Org / Company Code box kept rendering wider than the search boxes through both 0460 and 0461, even once their own `width` numbers matched exactly: `.readonly` is a `<div>` at the browser's default `content-box`, `tokens.css`'s own `box-sizing: border-box` rule only reaches `input`/`textarea`, so a content-box `width: 100%` plus `.readonly`'s own 22px of padding rendered 22px past the column it was meant to fill — reported live, with two screenshots, as *"way out of line again... it spills out of the page"* — fixed with one `box-sizing: border-box` on `.popout.codingpopout .codingcompanycode` alone) is confirmed pushed and deployed — `origin/main` fetched directly reads `1b64da3`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* No migration — `vf-ui` only, no new string key.** **Decision 0461 (the Coding pop-out's own search boxes and the read-only Org / Company Code box now both read `width: 100%` of one shared `--codingfield-w` grid column instead of each stating its own `66.6667%`, and the pop-out's own width is computed from that column plus a `--codingfield-label-w` label column rather than fixed at 900px — 190px / 320px, chosen live from the mock-up's own slider — so Close lands directly above the box's right edge as arithmetic, not a third number kept in sync by hand) is confirmed pushed and deployed together with decision 0462 in the same report — its own `width: 100%` turned out not to be enough of the fix on its own, see decision 0462 just above for why. No migration — `vf-ui` only, no new string key.** **Decision 0460 (every Coding pop-out field shows its own first page on `focus`, not only the one field auto-focused on open, and no minimum length before typing filters the shared results; search boxes narrowed to 2/3 of their own column, and the read-only Org / Company Code box matched to the same height and width) is confirmed pushed and deployed — `origin/main` fetched directly reads `52e3d23`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* No migration — no new string key, so nothing further to apply. **Live afterward, the operator reported the narrowing itself is not yet right: the box is still too wide, Close does not sit above the box's own right edge, and Org / Company Code is still both wider and taller than the boxes beneath it — see `docs/design/mockups/line-coding-popout-widths.html`, a mock-up (not built) working out the numbers and a structural fix (one shared box width driving both the grid and the pop-out's own size, rather than two separately-stated copies of it) before this becomes decision 0461.** **Decision 0459 (auto-focus and pre-load Cost Centre in the Coding pop-out, its results area back to one column instead of 0458's own two, and — General Ledger Code's own reported "no rows" — traced to intended behaviour per decision 0355's own "a missing filter is not everything" and a dedicated existing test, with a new "Narrowed by: <filters>" note on a scoped field's own empty result via migration `0157`) is confirmed pushed and deployed — `origin/main` fetched directly reads `e02f187`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed"*, including migration `0157`'s own separate apply. **The General Ledger Code question this decision raised is still open**, and needs the operator to check something this session cannot: whether those entries actually have Company Code / Commodity Code values set under AP Setup's own "Filter by" columns — the pop-out's own new "Narrowed by:" note should now say directly, on the operator's own next search there.** **Decision 0458 (the Coding pop-out is a fixed size — `.popout.codingpopout` — with one shared results area instead of one per field, `codingResultsController`'s generation-token guard against a stale field's slow search overwriting a faster one, a 2-column unwrapped results grid, and one new string via migration `0156`) is confirmed pushed and deployed — `origin/main` fetched directly reads `1d8c423`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed"*, including migration `0156`'s own separate apply.** **Decision 0457 (Account Coding Suggestions, Phase 1 — a frequency-based default computed from the invoice's own supplier history in `keyed_fields`, a new `coding-suggestions.ts` module and `GET /invoices/:id/coding-suggestions` route in `vf-app`, `viewer.js`'s pop-out pre-filling an empty field with a visibly-labeled suggestion in `vf-ui`, and one new string via migration `0155` in `vf-licence`) is confirmed pushed and deployed — `origin/main` fetched directly reads `2b4c520`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed"*, including migration `0155`'s own separate apply.** **Decision 0456 (six more routes on the Coding task path — `GET /invoices/:id`, its document/pages routes, `/progress`, and `POST /invoices/:id/key` — widened to also accept `AP.Code`) is confirmed pushed and deployed together with decision 0457 in the same report — see just above; it needed no migration of its own.** **Decision 0455 (`index.ts`'s `GET /org/cost-centres` and `GET /coding-lists/:type` widened once more to also accept `AP.Code`) is confirmed pushed and deployed together with decisions 0456 and 0457 in the same report — see above.** `93b4568` — fetched directly by this session, matching this session's own decision 0454 commit exactly. **Decision 0454 (`onTaskCompleted` no longer silently strands an invoice on a stage with a rule set attached but nothing evaluated) is confirmed pushed and deployed — `wrangler deploy` confirmed run for `vf-app`, the only worker it touched, and no migration to apply.** `ccbbc82` was decision 0453's own commit (the invoice-line Coding pop-out), confirmed pushed, deployed, and its migration applied — `wrangler deploy` confirmed run for all three workers it touched (`vf-app`, `vf-ui`, `vf-licence`), and migration `0154` confirmed applied, per the operator's own report.** `7e2d4ed` was decision 0452's own commit (Phase 2 of the Cost-Object Approval Hierarchy), confirmed pushed, deployed, and both its migrations applied — `wrangler deploy` confirmed run for both `vf-app` and `vf-licence`, and migrations `0077`/`0153` confirmed applied via the operator's own `apply_migrations.py --remote` output. Decisions 0434 (org placement and supplier matching now reach the first stage visit) and 0435 (a stage visit error is recorded, not swallowed) are confirmed pushed and deployed. **Decisions 0436 through 0438 (the sidebar fix) are confirmed pushed and deployed — the operator reported *"deployed and pushed - works great"*.** **Decision 0439 (Approval Hierarchy — the migration and the resolver) is confirmed pushed and deployed — the operator reported *"pushed and deployed"*.** **Decision 0440 (the AP Setup screen and the Approval Hierarchy write API) is now also confirmed pushed and deployed — the operator reported *"pushed and deployed"*.** **Decision 0441 (the proxy-allowlist fix for AP Setup's own routes) is confirmed pushed and deployed — `origin/main` fetched directly reads `3aadf95`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."*** Deploying it alone did not resolve the original report — see the `vf-app-poc` migrations row below for the second, unrelated cause found once the proxy itself was confirmed working, now also fixed and confirmed by the operator's own *"that worked - thank you."* **Decision 0442 (the override lists reordered below their own add-row forms, and made searchable) is confirmed pushed and deployed — the operator reported *"deployed and pushed."*** Its own migration `0146` was confirmed applied shortly after — the operator reported *"yes, I see the proper text hints now."* **Decision 0443 (the Add button becomes an icon button, top-right of its own card) is confirmed pushed and deployed — the operator reported *"pushed and deployed - thank you"*, and `origin/main` fetched directly reads `9655bfe`, matching this session's own commit exactly.** **Decision 0444 (Account Coding — the five coding lists' own generic framework and screen) is confirmed pushed and deployed — `origin/main` fetched directly reads `6c2f8d9`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."*** Deploying it alone did not resolve the report of the same live failure decision 0441 already named once — see the `vf-app-poc` migrations row below for the same root cause recurring a second time, now also fixed and confirmed by the operator's own *"that worked - thank you."* — see the paragraph above the "Where things stand" table for the full write-up. **Decision 0445 (CSV Template and Load for Cost Centre, Project, Commodity Code, and General Ledger Code) is confirmed pushed and deployed — `origin/main` fetched directly reads `81af146`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."*** Migration `0148` still needs its own separate apply — see the `vf-licence-poc` migrations row. **Decision 0446 (search and real, server-side pagination for the Account Coding tab's four manageable tables — Cost Centre, Project, Commodity Code, General Ledger Code) is confirmed pushed and deployed — `origin/main` fetched directly reads `3f530f6`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed - looks good."*** Tasks and Documents were explicitly asked about and deferred to a separate future decision (see `docs/decisions/0446-search-and-pagination-for-account-coding.md`), since neither one's current architecture can take the same treatment without a larger rewrite first. **Migration `0149` (two new UI-string keys) still needs its own separate apply — see the `vf-licence-poc` migrations row** — per the now-repeated habit this project keeps needing stated explicitly: a deploy and a migration apply are two separate steps here, and skipping the second produces raw-string-key UI, not an error. **Decision 0447 (Template/Load button strings reused from Purchase Orders; Access's "Org units" and AP Setup's "Company code" tabs renamed together to "Org / Company Code" via migration `0150`) is confirmed pushed and deployed — `origin/main` fetched directly reads `78357d6`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."*** Neither part needed a production-code change beyond `coding-lists.js`'s two button labels; the naming alignment is a value-only migration, following migration `0084`'s own UPDATE-based rewording precedent. **Migration `0150` (and the still-outstanding `0149` from decision 0446) still need their own separate `apply_migrations.py --remote` run — see the `vf-licence-poc` migrations row.** **Decision 0448 (real, server-side search and pagination for the Documents screen — Tasks and Documents' own split-off from decision 0446, Documents half only) is confirmed pushed and deployed — `origin/main` fetched directly reads `91eebe9`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed - looks great I have checked."*** No new migration. **Decision 0449 (search and real, server-side pagination for the Tasks screen — Tasks and Documents' own split-off from decision 0446, Tasks half, the harder of the two since Tasks' own visibility check walks unit lineage upward per-task rather than downward via a precomputed set) is confirmed pushed and deployed — `origin/main` fetched directly reads `4577011`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed"* and, separately, *"tested as working."*** One new migration, `0151`, still needs its own separate apply — see the `vf-licence-poc` migrations row. **Decision 0450 (Cost-Object Approval Hierarchy — investigation and a static mock-up of a Cost-Object Priority panel added to Approval Hierarchy and an Approval Limit column added to Account Coding's three greenfield lists, per the operator's own request to investigate and mock up rather than build) is documentation only — nothing to deploy — and is confirmed pushed: `origin/main` fetched directly reads `6e996c9`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."*** The operator also confirmed the design's own core principle directly: *"If cost object approval is needed on the invoice line, the invoice should be routed to the budget holder for that cost center"* — exactly what decision 0439's Cost-Object mode already does, confirmation of the base case rather than a change to it. See `docs/design/cost-object-approval-hierarchy.md` and `docs/design/mockups/cost-object-approval.html`; three questions are still named there as genuinely open for the operator to answer before any of it becomes a real build. **Decision 0451 (Line Level Account Coding — Phase 1 of the Cost-Object Approval Hierarchy plan: three new vocabulary fields, `coding.project`/`coding.commodity_code`/`coding.gl_code`, reaching the existing generic keying, field-visibility, and viewer mechanisms with no route or screen code change) is confirmed pushed and deployed — `origin/main` fetched directly reads `c25ff2b`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed and sql applied"* — all three in one report, including migration `0152`'s own separate apply step.** Judged, not asked directly — the operator's own question in the previous round (*"Does it make sense to do that before the Cost Center approval routing"*) is answered yes in `docs/decisions/0451-line-level-account-coding.md`, since Phase 2's own resolver generalization has nothing real to route on without this first. |
...
tested, and documented, not yet committed/pushed at the time of
writing.** Reported live against a real `assign_task` permission bug
...
all new. Full reasoning in decision 0478. Not yet confirmed
pushed/deployed by the operator as of this writing — this paragraph
will be corrected once they confirm, the same two-step pattern every
other decision in this file follows.**
