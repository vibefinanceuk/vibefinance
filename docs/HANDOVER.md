...
23 September (thirty-five times), updated 24 September (twenty-nine times).**
...
| | |
| --- | --- |
| `origin/main` | **Decision 0478 ("Here because" — why a task landed on you) is confirmed pushed and deployed** — `origin/main` fetched directly reads `c48882f`, matching this session's own commit exactly. A rule-attributed reason line above the stage progress bar, kept structurally separate from `workflowErrorPanel` (decision 0435): that names an engine failure, this names an ordinary, working rule outcome. Placement (a slim line above the stage progress bar, Option A) chosen by the operator from a live interactive mockup shown before any code was written. A rule's own *name* is translatable (the four standard matching rules through `ui_strings` by their stable `key`; a customer's own authored rule name through a new `rule_name_translations` table) but its full *sentence* never is. A real, pre-existing gap fixed alongside this: `workflow-engine.ts`'s own `assign_task` handling had always discarded which rule fired a task, despite `evaluateRuleSet` always knowing it — no task anywhere had ever recorded this until now (`tasks.rule_id`, migration `0079`). Not retroactive: a task already open before the deploy shows nothing, by design — this is exactly what the operator first hit (*"I don't see the change in the UI yet"*), resolved once they cleared runtime data and resubmitted test invoices: *"confirmed - I cleared out data again and resubmitted the previous invoices. I see the message now."* `vf-app` `workflow-engine.test.ts` **60/60**, `rules-list.test.ts` **43/43**, `key-fields.test.ts` **56/56**, plus 934 further tests across every file touching `tasks`/`rules`, all green except one pre-existing unrelated failure. `vf-licence` migration chain (163) replays clean. `vf-ui` browser suite **1090/1091** (one pre-existing unrelated failure), `rule.test.ts` **26/26**, `viewer.test.ts` **186/186**, all new. Full reasoning and verification counts in decision 0478. |
...
every panel claiming it whole) is built, tested, documented, not yet
committed/pushed at the time of writing.** Reported live, cautiously,
