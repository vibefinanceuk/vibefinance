...
(twenty-five times), updated 22 September (five times).**
...
| `vf-licence-poc` migrations | through `0145` applied and confirmed live — the operator's own `apply_migrations.py --remote` run, `0144` is decision 0435's own banner-label string (`viewer.workflow.stageerror`, en/de). **`0145` (decision 0440's own AP Setup strings) is now applied and confirmed live too.** **`0146` (decision 0442's own override-list search strings) is built and tested, replays clean, not yet applied to `vf-licence-poc`.** |
| Tests | vf-admin 9 · vf-app **2534** (2513 + 21 `approval-config-route.test.ts`, decision 0440, confirmed by one unfiltered whole-suite run) · vf-licence **320** (unchanged in count — decisions 0440's own migration 0145 and 0442's own migration 0146 both add rows to the existing `ui_strings` table, not new test files) · vf-ui 74 Worker (unchanged since 0440 — no proxy/route change since) + **978** browser (960 + 13 `ap-setup.test.ts` for decision 0440, + 5 for decision 0442's own reorder-and-search tests) · shared 295 (+3 known pre-existing failures) |
| Decision records | 442 |
...
and gain search) is built and tested. Not yet pushed or deployed.**
Raised directly by the operator, once decision 0441 closed out the
