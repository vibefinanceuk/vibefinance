...
(twenty-five times), updated 22 September (six times).**
...
| `vf-licence-poc` migrations | through `0145` applied and confirmed live — the operator's own `apply_migrations.py --remote` run, `0144` is decision 0435's own banner-label string (`viewer.workflow.stageerror`, en/de). **`0145` (decision 0440's own AP Setup strings) is now applied and confirmed live too.** **`0146` (decision 0442's own override-list search strings) is built and tested, replays clean.** **Very likely not yet applied to the real `vf-licence-poc`** — once 0442 deployed, the operator reported the two new search-hint strings showing as raw, untranslated keys (`apsetup.supervisoroverridesearchhint`, `apsetup.limitoverridesearchhint`) rather than their real text. `t()` returns the raw key when a string is missing from what loaded (`strings.js`'s own documented behaviour) — both keys match `ap-setup.js`'s own calls and `0146`'s own rows exactly, so this reads as the same shape decision 0441's own second finding already was: "deployed and pushed" confirms the worker code, not that the migration was separately applied. Not yet confirmed either way by a direct query, the way `0075`'s own gap was — named to the operator as the likely cause and the fix (`apply_migrations.py --remote --database vf-licence-poc --migrations-dir workers/vf-licence/migrations`) rather than assumed silently. |
...
its own card) is built and tested. Not yet pushed or deployed.** Asked
directly, from a screenshot, once decision 0442 deployed: *"create a
...
already was — see the `vf-licence-poc` migrations row above. New: one
...
migration `0146` appears not to have been applied yet** — see the
`vf-licence-poc` migrations row for the evidence and the fix. Raised
