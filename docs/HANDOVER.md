...
23 September (thirty-five times), updated 24 September (twenty-nine
times), updated 25 September (five times).**
...
| | |
| --- | --- |
| `origin/main` | **Decision 0506 (`.vtimeline` respects `hidden` again) confirmed pushed and deployed** — `origin/main` fetched directly reads `bb1b469`, which is this decision's own commit; the operator confirmed with “deployed and pushed, that worked thank you.” Reported live, against two screenshots taken right after decision 0505 was confirmed working: with the Document tab selected, the System Alert (“This document could not be read automatically...”) stayed on screen and hovered over the Invoice Lines card beneath it, instead of only showing inside Timeline / Chat. **A regression from decision 0504, minutes earlier in this same session, and the exact bug class this codebase had already found and fixed twice before** (decision 0271, for `.backdrop`; the existing `.activitytabcontent[hidden]` rule, for a different element): `timelinePane`/`.vtimeline` had no class, and so no `display` of its own, until 0504 gave it `display: flex` to make it a real flex column with a real height — which is exactly what silently overrides the browser's own default `[hidden] { display: none }` the moment `select()` (`viewer.js`) sets `hidden` on it for the Document/XML tabs. The `hidden` attribute itself was still being set correctly the whole time (every existing test asserting `.closest("[hidden]")` on this pane still passed) — nothing in the CSS honoured it any more. **Fix**: the same established pattern, restated at the same selector scope that introduced the regression — `.c-document .vtimeline[hidden] { display: none; }`. No change to decision 0504's own work (the real height stands), the alert's own wording or position, or anything else about the pane's layout — this restores exactly the hide/show behaviour that already existed before 0504, now compatible with the real height it also needed. Tests: `workers/vf-ui` — a new stylesheet-reading test (the same jsdom-applies-no-CSS pattern the existing `.activitytabcontent[hidden]` test already uses) confirming the new rule exists and resolves to `display: none`; full browser suite **1185/1186** (the one failure the same confirmed pre-existing, unrelated `typography.test.ts` hardcoded-`10px` gap); plain suite **75/75**, unchanged. `npx eslint` clean on the touched test file. Full reasoning in decision 0506. **Decision 0505 (The pop-out window stops growing past the screen) confirmed pushed, deployed, and tested working** — `origin/main` fetched directly reads `e9f8e48`, which is this decision's own commit; the operator confirmed with “deployed and pushed, and tested working!” Reported live: in the expanded document viewer, adding several messages made “the height of the page grows and grows” instead of scrolling the Timeline / Chat internally the way the docked, embedded card already did, with a request that the card fill the browser window and grow a scrollbar inside itself once content outgrows that space, rather than expanding the page. **Two bugs in the pop-out's own outermost chrome, not in the Timeline / Chat pane itself**: `body.docwindowbody` had `min-height: 100vh` — a floor, not a ceiling, so the page grew to fit its content instead of clipping anywhere — and `#docwindow-root` had no `min-height: 0`, the classic flex-item gotcha where the default minimum height is the content's own intrinsic size, not zero, so even a genuinely fixed `height` on `body` would not have stopped this box refusing to shrink. Everything below it (`.docwindowsplit`/`.docwindowsplitright`/`.docwindowtimeline`/`.activitytabcontent`) already carried a correct `flex: 1; min-height: 0` chain several decisions deep (0384, 0386, 0392, 0394) ending in a real `overflow-y: auto` — it simply had no ceiling to compute against, because its own outermost ancestor never gave it one. **Very likely surfaced by decision 0504, moments earlier in this same session**: before that decision, `.activityfeed` had a flat `max-height: 300px`, masking this outer-chrome defect by capping the feed on its own regardless of how tall the page was ever allowed to get. **Fix**: `height: 100vh; overflow: hidden` on `body.docwindowbody` in place of `min-height: 100vh`; `min-height: 0` added to `#docwindow-root`. No change to anything below that — the existing chain was already correct and needed only a real ceiling to bound against. Now the card fills the browser window, the reply box stays fixed at the bottom, and the entries scroll internally once they no longer fit. Tests: `workers/vf-ui` — a new stylesheet-reading test (the same jsdom-applies-no-CSS pattern this file's neighbours already use) confirming both rules are in place and the old `min-height: 100vh` is gone; `document-window.test.ts`/`viewer.test.ts`/`ap-setup.test.ts` together **321/321** (1 new); full browser suite **1184/1185** (the one failure the same confirmed pre-existing, unrelated `typography.test.ts` hardcoded-`10px` gap); plain suite **75/75**, unchanged. `npx eslint` clean on the touched test file. Full reasoning in decision 0505. **Decision 0504 (The Timeline / Chat pane keeps its own height) confirmed pushed and deployed** — `origin/main` fetched directly reads `15232e7`, which is this decision's own commit; the operator confirmed with “deployed and pushed.” Reported live, against two screenshots of the same task at AP Review: the reply box sat low against the Timeline / Chat card's own bottom edge, and opening “Add person” pushed the whole feed — reply box included — down into the Invoice Lines card beneath it, with a request that the reply box and Post button keep a fixed position at the card's own bottom regardless of how many entries are above it. **Root cause found by tracing the height chain, not guessed**: `viewer.js`'s own `timelinePane` (the collaborator bar, the optional System Alert, and the activity feed) had no class of its own, so it never reached `.c-document`'s existing height rules — the same ones `.vpreview` already relies on for the Document/XML tabs (decision 0391, which fixed this exact overlay-into-the-next-row bug once before, just for a different pane). Left in plain block flow, it grew with whatever its own content added up to, free to overflow the card. **Fix**: gave `timelinePane` a class (`vtimeline`) and the same `height: 100%; min-height: 0; display: flex; flex-direction: column` shape `.vpreview` already has, with the collaborator bar and System Alert keeping their own natural size (`flex: 0 0 auto`) and `.activitytabcontent` taking whatever is left (`flex: 1 1 auto; min-height: 0` — the same “last one grows” shape decision 0478 already used for a stack of unequal panels sharing one row). Inside it, `.activityfeed` changed from a flat `max-height: 300px` to `flex: 1; min-height: 0`, so the entries scroll and give way, not the reply box; `.activityinput` got an explicit `flex: 0 0 auto`, pinning it — exactly the fixed-position behaviour asked for. The narrow-screen fallback (`@media (max-width: 1100px)`) got the matching `calc(100vh - 300px)` `.vpreview` already uses there. Layout only — no change to what the feed shows, how comments post, or collaborator search itself; `.activityfeed`'s new rule is shared code, so the pop-out window's own feed (decision 0394) now also fills real available space instead of a flat 300px cap, a strict improvement there too. Tests: `workers/vf-ui` — a new stylesheet-reading test (the same jsdom-applies-no-CSS pattern decisions 0257/0261/0271 already established) confirming `.vtimeline` renders and all three new rules are in place; `viewer.test.ts`/`document-window.test.ts`/`ap-setup.test.ts` together **320/320** (1 new); full browser suite **1183/1184** (the one failure the same confirmed pre-existing, unrelated `typography.test.ts` hardcoded-`10px` gap); plain suite **75/75**, unchanged. `npx eslint` clean on every touched file. Full reasoning in decision 0504. **Decision 0503 (Remove the activity panel's “Internal only” footer) confirmed pushed and deployed** — `origin/main` fetched directly reads `34af297`, which is this decision's own commit; the operator confirmed with “deployed and pushed, migrations applied.” A one-line request, reported right after decision 0502's own confirmation: remove the sentence at the foot of the Timeline/Chat panel reading “Internal only — not visible to the supplier.” **Design untouched, only the sentence removed** — the words came from `activity.internalonly` (`vf-licence` migration `0078`), seeded when the activity panel was first built (decision 0267), whose own record and `document_comments`'s own migration (`0059`) both establish this as a design property, not a label: comments have no supplier-facing column and no visibility flag to toggle, so nothing about *whether* a comment reaches a supplier changed here — only whether the panel says so out loud. Removed the `.activityfoot` div and its `t("activity.internalonly")` call from `activity.js`'s own `renderContent()` (the panel's one render site — `document-window.js` reuses it rather than duplicating it, so nothing else needed touching), the now-unused `.activityfoot` CSS rule, and the now-stale `"activity.internalonly"` entry in `string-coverage.test.ts`'s own hand-maintained `KEYS_THE_INTERFACE_USES` list. The seeded string itself was left in place — this codebase does not rewrite old migrations, and an unused seeded row is harmless. Tests: `workers/vf-ui` — `viewer.test.ts`/`document-window.test.ts` (browser) **250/250**, unchanged from before this change, confirmed via `git stash` that the same 287 pre-existing, unrelated unhandled-rejection warnings (an unstubbed `/collaborators` route) appear identically with and without it; `workers/vf-licence` — `string-coverage.test.ts` **10/10**. `npx eslint` clean on every touched file. Full reasoning and verification counts in decision 0503. **Decision 0502 (Discard gets a real picker, a stage restriction, and a Timeline check) confirmed pushed and deployed** — `origin/main` fetched directly reads `3195468`, which is this decision's own commit; the operator confirmed with “deployed and pushed, migrations applied.” Four questions reported live, once decision 0501's own fix was confirmed working: the Discard reason prompt still read as a native browser dialog, not this site's own; whether a discard was actually tracked in Timeline/Chat; whether Discard could be restricted to the Validation queue only, since nothing stopped it once a document had been coded, matched, or approved; and what discarding does to the instance's own status. **Investigated before building, not assumed** — the Timeline mechanism (decision 0488) was already correct and directly tested (`activity-route.test.ts`'s own “derives a discard entry”), rendered by the exact same shared code path Release and Return already use; the one real gap was in test coverage (nothing exercised a discard's own reason text rendering at the browser level), not behaviour, and was closed here rather than treated as a bug fix. The stage restriction, though, was genuinely missing: `AP.Discard` plus the task's own stage permission was the *whole* gate, with no notion of *which* stage at all — a person holding it could discard a document already validated, coded, matched, or approved. **Three fixes**: `window.prompt()` retired for Discard — its own dedicated `.backdrop`/`.popout` picker (`openDiscardPicker`, `viewer.js`), the same minimal shape `openReturnPicker` already uses for its own required reason, just with no target to choose, checked client-side before ever calling the server so the most common way to get it wrong costs no round trip. A new `discard_allowed` column on `stage_actions` (migration `0091`) — `'discard'` was already reserved in that table's own `KNOWN_ACTIONS` list since decision 0487, with no column of its own until now, exactly the extension point that decision's own migration comment anticipated. **Defaults to allowed everywhere — the `offer_field_restrictions` (0081) direction, not `reverify_rule_on_complete`'s (0082) own opposite one** — turning this off silently removes a button already available today, so nothing changes on deploy until an operator explicitly restricts a stage. Enforced both server-side (`handleDiscard` itself refuses, 409, checked ahead of the reason requirement) and client-side (the button is never offered, computed once per distinct stage the same way `offerRouteToApprover` already is, and only even queried for someone who holds `AP.Discard` in the first place). Surfaced as a new checkbox on the existing Stage Restrictions tab (`setStageDiscardAllowed`, `ap-setup.js`), alongside the reverify-on-complete toggle, independent of the Account Coding restrictions above it. `vf-app`, `vf-ui`, and `vf-licence` (new migration `0178`, `action.discard.reasonlabel`/`apsetup.stagerestrictions.discardallowed`, en/de) all touched — no change to `endTaskAndSiblings`, `process_instances.status`, or `current_stage_id`; decision 0078's own terminal-state design (the instance reaches `archived`, the stage stays as history) was never in question, only restated, and — since decision 0501 — the Documents list already reads “Archived” for a discarded invoice too, not just a returned one. Tests: `vf-app` — `stage-actions-route.test.ts`/`return-route.test.ts`/`task-list-route.test.ts` together **125/125** (22 new); `process-route.test.ts` **73/73** (2 new); full suite, batched across 119 files, **2983/2985** (the 2 failures the same confirmed pre-existing, unrelated `capture-pdf.test.ts`/`stage-permissions.test.ts` gaps); `apply_migrations.py --replay-only`: 91 migrations, all assertions held. `vf-licence`: `string-coverage.test.ts` **10/10** (2 new keys). `vf-ui`: `viewer.test.ts` (browser) — six new Discard-picker tests plus one new Timeline-comment test, all passing; `ap-setup.test.ts` (browser) **69/69** (6 new); full browser suite **1182/1183** (the one remaining failure the same pre-existing, unrelated `typography.test.ts` gap); plain suite **75/75**, unchanged (the `/actions/:action` proxy pattern was already generic enough to need no allowlist change). **Still to do, operator side: in AP Setup → Stage Restrictions, uncheck “Allow Discard at this stage” for Coding, Matching, and Approval, leaving Validation checked** — that toggle is the actual fix for the business rule asked about; nothing here restricts any stage on its own. Full reasoning and verification counts in decision 0502. **Decision 0501 (The Documents list learns `returned_manually` and `archived`) confirmed pushed and deployed** — `origin/main` fetched directly reads `a9e2070`, which is this decision's own commit; the operator confirmed with “deployed and pushed, migrations applied, tested and fixed. Document now read - Returned to supplier.” Reported live, right after decision 0498's Return To Supplier button was used for the first time: “I placed the Return to Supplier button, but it continues to sit in the same queue it seems. Although gone from my task list, the item when searching for Documents, appears to still be in the matching Matching stage.” Checked directly, not assumed: `handleReturnToSupplier` (`return-route.ts`) already did exactly what it should — ended the task and set `process_instances.status = 'returned_manually'`, with `current_stage_id` deliberately left untouched per decision 0055 section 7's own design (the stage a document was returned from is a real historical fact, not something a terminal transition erases). The bug was one layer up: `documents-route.ts`'s own `statusOf()` predates decision 0055's two terminal statuses by many decisions (0164 vs. 0055/0498) and had never been taught either of them, so a `returned_manually` instance fell straight through to the stage-based fallback and read as “Waiting”/“In progress” at whatever stage it left from — precisely the false impression reported. **Genuinely untested before this** — no existing test covered `"waiting"`, `"moving"`, or `"done"` at all, letting the gap go unnoticed since the function's creation. Fixed by adding `returned_manually` → `"returned"` and `archived` → `"archived"` ahead of the stage-based fallback, both taking priority the same way `"completed"` already does; `current_stage_id`/`stageName` are left exactly as they are — the stage name next to the now-honest status reads as history, not as a claim about the present, so no separate UI decision about hiding or relabelling the stage cell was needed. New status colours in `vf-ui`'s `app.css` (`.status.returned` reusing the Return To Supplier action's own accent colour; `.status.archived` quieter than `moving`/`outside`, matching an already-used `--text-muted`), migration `0177_docstatus_returned_archived.sql` (`vf-licence`, `docstatus.returned`/`docstatus.archived`, en/de). `vf-app`, `vf-ui`, and `vf-licence` all touched — no change to `return-route.ts` or the underlying data, and no change to the dashboard's own “Where things are” card or any other `status = 'in_progress'`-scoped view, since those already exclude terminal instances by construction. Tests: `vf-app` `documents.test.ts` **69/69** (3 new); full suite, batched, **2965/2967** (the 2 failures the same confirmed pre-existing, unrelated `capture-pdf.test.ts`/`stage-permissions.test.ts` gaps). `vf-licence`: `string-coverage.test.ts` **10/10** (2 new keys); full suite **320/320**, unchanged. `vf-ui`: `documents.test.ts` (browser) **36/36** (2 new) — this tab's first-ever status-word coverage; full browser suite **1174/1175** (the 1 remaining failure the same pre-existing, unrelated `typography.test.ts` gap); plain suite **75/75**, unchanged. Full reasoning and verification counts in decision 0501. **Decision 0500 (Return Reasons' own row, given a layout built for it) confirmed pushed and deployed** — `origin/main` fetched directly reads `4611f22`, which is this decision's own commit; the operator moved straight on to reporting decision 0501's own bug without a separate confirmation word for this one, so this row records it now, alongside 0501, on the strength of `origin/main` itself already holding the commit. Raised live, against decision 0499's own checkbox-size fix: two screenshots showed the checkbox now correctly small, but each Return Reasons row still wrapped onto two visual lines. Asked in general terms first — “do you have any capacity for interface design?” — then, offered a proactive fix with a choice of mocking it up first or building it directly, the operator answered “... yes, please,” choosing to build directly. **Root cause: the row was never a genuine `.editgrid` use** — that shared class is a 2-column grid built for label-above-value *pairs*, but the Return Reasons row holds three or four unrelated controls (label, checkbox, active text, save action), which wraps onto two lines in a 2-column grid regardless of how any individual control is sized — decision 0499's own checkbox-sizing fix could never have solved this by itself. Fixed with a purpose-built `.returnreasonrow` (flex row, the same shape this codebase's own `.assignmentrow` already uses for “label left, one action right, one row per list item”), not another `.editgrid` variant. `returnReasonRow(reason, problem)` extracted as its own function (previously inline in a `.map()`), with the checkbox and its “Active” text combined into one clickable `<label class="returnreasonactive">` wrapping both. The add-row reuses the same `.returnreasonrow` class with a narrower first field (`.returnreasonnew`), so the whole tab now reads as one consistent list rather than a grid awkwardly repurposed. The AP team email panel beneath it is unaffected, still a genuine `.editgrid` label/value pair. `vf-ui` only — CSS and `ap-setup.js`, no `vf-app`/`vf-licence` change, no migration. **This tab's first-ever browser test coverage**: `test-browser/ap-setup.test.ts` gained a new describe block, 7 tests — one row per reason (not split across two lines), each row reflecting its own reason's checked state, clicking the label toggling the checkbox, saving the correct reason's edited label and active flag, the add-row rendering as its own single line with a narrower ID field, posting a new reason reloading the list, and the AP team email panel rendering unaffected. Full reasoning and verification counts in decision 0500. **Decision 0499 (Return Reasons' own “Active” checkbox, stretched by `.editgrid`'s own width rule) confirmed pushed and deployed** — `origin/main` fetched directly reads `95ca4f5`, which is this decision's own commit; the operator confirmed with “pushed and deployed.” Reported live from two screenshots of decision 0498's own new Return Reasons tab, right after that decision went live: “The check box on return reasons seems kinda large.” `.editgrid > input { width: 100%; }` had no checkbox exclusion, so the native checkbox stretched to fill its whole grid column — the same bug decision 0282 already fixed once, for `.columnlist`'s own checkboxes. Fixed at the shared `.editgrid` rule itself (14×14px, matching 0282's own values), not per-screen, which also quietly fixes two pre-existing, never-reported instances of the identical bug in `coding-lists.js` and Account Coding's own number-limit checkbox. CSS-only — no `vf-app`/`vf-licence` change, no migration. Full reasoning and verification counts in decision 0499. **Decision 0498 (Return To Supplier: reasons, a comment, and a real email) confirmed pushed and deployed** — `origin/main` fetched directly reads `364e51c`, which is this decision's own commit; the operator confirmed with “deployed and pushed” and, asked directly, confirmed migrations applied — `vf-app`'s own `0087`–`0090` and `vf-licence`'s own `0176_return_to_supplier_picker_strings.sql`. Reported live, framed as analysis first: a set of auditable return reasons, a comment box for the supplier, showing the communication path (the supplier's own email; the invoice's own contact email deferred), an AP-team CC checkbox, and whether a returned item needed a new Archive stage to stop it sitting in Payment-eligible. The Archive-stage question was already answered by decision 0055 section 7's own two terminal instance statuses (`returned_manually` / `archived`), confirmed as a non-issue with no code change needed. **Supersedes one sentence of decision 0055 section 7** — “the system does not send anything back to the sender” — the moment an AP-team CC checkbox was asked for, since copying an email in only makes sense once an email is genuinely being sent; the rest of section 7 stands untouched. Resend chosen over Cloudflare's own native Email Service (Beta, Workers-Paid-only, destination-restricted), confirmed directly before building, with full delivery/bounce tracking via Resend's own webhook posting directly to `vf-app`'s public `workers.dev` address (deliberately never added to `vf-ui`'s proxy allowlist, since that proxy needs a session cookie a webhook cannot supply). The terminal transition always succeeds independent of the email's own outcome, which is tracked as its own best-effort row in the new `supplier_return_emails` table. Full reasoning and verification counts in decision 0498. **Decision 0497 (Route To Approver's own optional comment, and picker spacing) confirmed pushed and deployed** — `origin/main` fetched directly reads `6382605`, which is this decision's own commit; the operator confirmed with “deployed and pushed” and, asked directly, confirmed migration `0175_route_to_approver_comment.sql` applied (alongside `vf-app`'s own migration `0086`), and directly reported it “tested, and looks great.” Full reasoning and verification counts in decision 0497. **Decision 0496 (Route To Approver's own candidates route, missing from vf-ui's proxy allowlist) confirmed pushed and deployed** — `origin/main` fetched directly reads `e0dcb08`, which is this decision's own commit; the operator confirmed with “deployed and pushed” and directly reported the picker now opens with available users, confirming the fix live. Touches only `vf-ui` — no `vf-app` or `vf-licence` redeploy needed, no migration involved. Full reasoning and verification counts in decision 0496. **Decision 0495 (Route To Approver, conditional on Manual mode) confirmed pushed and deployed** — `origin/main` fetched directly reads `1fd3523`, which is this decision's own commit; the operator confirmed with “deployed” and, asked directly, confirmed migration `0174_route_to_approver_strings.sql` applied. Reported live, as a direct correction to a research pass's own first conclusion: “Route to Approver should allow manual selection of an approver, if the AP Setup Approval Hierarchy is set to Manual. Otherwise, no selection of an approver, and follow the employee-supervisor or cost-center model.” The fifth and last item of the Coding-pilot sequence (0487). Resolution already happens exactly once, synchronously, when the prior stage's task is completed (`visitCurrentStage`'s own Approval Hierarchy dispatch, unchanged since 0439) — so “Route To Approver” is offered on that prior task and REPLACES `complete`, never sits beside it, exactly when the next stage uses Approval Hierarchy and the org is on Manual mode right now. It posts to the ordinary `POST /tasks/:id/complete` with an added `targetUserId`, not a new route. Candidates are every org-wide holder of the next stage's own `required_permission` — confirmed directly with the operator, since Manual mode has no unit or hierarchy to scope a picker by. Full reasoning and verification counts in decision 0495. **Decision 0494 (the supplier search pop-out's own actions, moved to the corner) confirmed pushed and deployed** — `origin/main` fetched directly reads `4a93b55`, which is this decision's own commit; the operator confirmed with “pushed” and, asked directly, confirmed vf-licence deployed with migration `0173_record_supplier_button_wording.sql` applied. Reported live: “There are two buttons on the pop-out, Record this supplier from the invoice, and close. Please can these be moved to the top right of the card / pop-out. Create an icon for the new button, and abbreviate the text to Record New Supplier.” `openSearch()` (shared by Change Seller and Change Buyer) now opens with a `.cardhead` — heading left, actions right, the same shape every other pop-out here already uses — instead of Close alone in the corner and Record as a plain `.secondary` button below the results. New icon `recordsupplier` (a document with the same offset “+” this app's other “add” icons already use), deliberately not `newseller`'s person-and-plus, which already means the *other* new-supplier action (`openNewSellerForm()`'s own blank form). Label reworded via `UPDATE` migration (`0173_record_supplier_button_wording.sql`, en/de): “Record this supplier from the invoice” → “Record New Supplier,” keeping decision 0233's own choice of “Record,” not “Create.” Tests: `vf-ui` `viewer.test.ts` 223/223 (three new — the cardhead's own button order, the Record flow posting then attaching the supplier, and Change Buyer's own pop-out showing Close alone); full unfiltered browser suite 1150/1151 (the 1 remaining failure is the same pre-existing, unrelated `typography.test.ts` finding). `vf-licence`: `string-coverage.test.ts` 10/10, full suite 320/320. Full reasoning and verification counts in decision 0494. **Decision 0493 (the pop-out alert, redrawn and reworded) confirmed pushed and deployed** — `origin/main` fetched directly includes `35f6f2f`; the operator confirmed with “pushed” and, asked directly, confirmed vf-licence deployed with migration `0172_reassign_nonefound_wording.sql` applied. Reported live, against 0492's alert actually deployed: “It looks unaligned, ugly, and the message itself is unhelpful. How about something like the attached, except saying ‘There are no eligible users to reassign’” — followed by “in an appropriate colour scheme” and “use an icon that is suitable and in line with the brand of this site.” `note()`'s alert now has its own layout (`.popout.notealert`: a large coloured icon, centred message, plain centred OK pill) instead of the picker-borrowed `.cardhead`/`.statebuttons`/`.actionlink` that read as unaligned. Two icon tones reusing existing, already-paired tokens: `systemalert`'s triangle in `--bg-warning`/`--text-warning` for every caller but one, `done`'s checkmark in `--bg-success`/`--text-success` for Save succeeding. Reassign's own “nothing eligible” message reworded via an `UPDATE` migration (`0172_reassign_nonefound_wording.sql`), en/de; Return's own message unchanged. Tests: `vf-ui` `viewer.test.ts` 220/220 (a new test for Save's own success-icon styling; three existing tests updated for the new selectors/wording); full unfiltered browser suite 1147/1148 (the 1 remaining failure is the same pre-existing, unrelated `typography.test.ts` finding). `vf-licence`: `string-coverage.test.ts` 10/10, full suite 320/320. Full reasoning and verification counts in decision 0493. **Decision 0492 (note() as a pop-out alert, requiring OK) confirmed pushed and deployed** — `origin/main` fetched directly reads `21da708`, which is this decision's own commit; the operator confirmed migration `0171_note_ok_string.sql` applied too (“Yes, deployed and migration applied”), and sent a screenshot of the alert live — the checkmark icon and “OK” button, dismissing a `viewer.actionfailed` (“That could not be done.”) alert on a real task screen. Asked directly, immediately after 0491's own delivery: “Rather than show the message in a discrete part of the page, would it be possible to alert it in a pop-out alert message on the page, with OK as the only option to click and acknowledge the message” — and, asked whether that meant only the two “nothing available” messages or every message `note()` shows, the answer was “Every message this note() function shows.” **Supersedes decision 0491's `scrollIntoView` fix before it ever shipped** — see `docs/decisions/SUPERSEDED.md`. All nine `note()` callers (Save success/failure, “nothing to save,” a blocked pop-up, the generic action-failed message, and Reassign/Return's own “nothing available” messages) now show the same `.backdrop`/`.popout` alert, dismissed only by its own OK button; the message keeps `id="viewer-note"`, and the old persistent placeholder div is removed. New string `action.ok` (“OK”), migration `0171_note_ok_string.sql`. Tests: `vf-ui` `viewer.test.ts` 219/219 (the pop-up-blocked test extended to also check the alert's OK-dismiss behaviour; the two “nobody eligible”/“no target configured” tests from decisions 0489/0490 rewritten from a `scrollIntoView` spy to asserting the pop-out alert itself); full unfiltered browser suite 1146/1147 (the 1 remaining failure is the same pre-existing, unrelated `typography.test.ts` finding). `vf-licence`: `string-coverage.test.ts` 10/10, full suite 320/320. Full reasoning and verification counts in decision 0492. **Decision 0491 (the viewer's own feedback note scrolls into view) confirmed pushed and deployed, then immediately superseded by decision 0492's own commit on top of it** — `origin/main` fetched directly includes `473c2f7`, and the operator confirmed with “okay - pushed.” Its `scrollIntoView` fix is no longer in the code note() runs today; decision 0492's pop-out alert replaced it in the very next commit. Reported live: “I cannot reassign an item when it is claimed by my user, but it works when not claimed.” Reproduced directly in the operator's own browser, driven this session: the candidate list really was empty (the operator is the only person on the team who holds the permission, correctly excluded as the current claimant per decision 0489's own rule), and `openReassignPicker` correctly called `note()` with “Nobody else on this team can take this task” — but `#viewer-note` sits in the very last row of `.columns`'s own CSS grid, below Document/Parties/Header/Lines, so a topbar action clicked before scrolling left the message real but invisible. No backend bug at all — confirmed via the Network tab showing `GET .../reassign-candidates` correctly returning `200 {"candidates":[]}`. **Fixed in `note()` itself, not as a Reassign special case** — Save's own success/failure and the generic action-failed message share the same box and the same risk for anybody who clicks a topbar action before scrolling down. `scrollIntoView` is called right after setting the message, guarded with `?.` since jsdom (this suite's own test environment) has no `scrollIntoView` at all — confirmed directly (`typeof el.scrollIntoView` is `"undefined"` there). Tests: `vf-ui` `viewer.test.ts` 219/219 (the two existing “nobody eligible”/“no target configured” tests from decisions 0489/0490 each extended with a `scrollIntoView` spy, no new test count since both were already regression coverage, just missing the visibility check); full unfiltered browser suite 1146/1147 (the 1 remaining failure is `typography.test.ts`'s own pre-existing, unrelated `app.css` finding, already confirmed present on an unmodified checkout in decision 0490's own verification). No `vf-app`/`vf-licence` change, no new migration — purely a client visibility bug, not a permissions or candidate-computation defect. Full reasoning and verification counts in decision 0491. **Decision 0490 (Return-target stage configuration, and the AP Setup screen for it) confirmed pushed and deployed** — `origin/main` fetched directly reads `30b50a0`, which is this decision's own commit, and the operator confirmed with "deployed and pushed, migrations applied." Third of the five-step Coding-pilot sequence agreed after decision 0487. Tracing the request found Return has never actually worked from the live UI: `POST /tasks/:id/return` has required a `stageId` plus an assignee since decision 0075, but nothing ever collected either — every click 400'd; this decision is what first makes it reachable. **A second, real bug fixed along the way**: the returned task's `required_permission` was inherited from the *returning* stage rather than resolved from the *target* stage, unlike every other task-creation path in the codebase — fixed with a lookup + fallback chain matching `workflow-engine.ts`'s own precedent. **New table `stage_return_targets`** (migration `0085`), not a `stage_actions` column — a stage can offer more than one return target; targets name a team, matching `assign_task`'s own default. **Server-computed candidate intersection**, the same philosophy decision 0489's own Reassign candidates used: a new `GET /tasks/:id/return-targets` intersects the admin-configured list with the document's own actual visit history. **A dedicated picker**, the same shape Reassign's own established — reason is mandatory (unlike Reassign's optional comment), left to the server's own 400 to enforce. **AP Setup's Stage Restrictions tab** gained a "Return targets" section per stage — configured list with Remove buttons, an add-row form shown only once another stage and a team both exist; the team list rides on the already-`Admin.Configure`-gated `GET /processes/:id` response rather than widening `/org/teams`'s own narrower auth. Tests: `vf-app` `return-route.test.ts` 28/28 (7 new), `stage-return-targets-route.test.ts` 11/11 (new file), `process-route.test.ts` 71/71 (3 new), `index.test.ts` in full 194/194 (5 new); full unfiltered suite 2935/2937 (the 2 remaining failures confirmed pre-existing and unrelated via `git stash` comparison). `vf-licence`: `string-coverage.test.ts` 10/10 (8 new keys, plus a pre-existing gap — `apsetup.add`/`roles.remove` — closed alongside them); migration chain 170, all assertions held. `vf-ui`: proxy allowlist `index.test.ts` 74/74 (3 new routes), browser `ap-setup.test.ts` 57/57 (9 new), browser `viewer.test.ts` 219/219 (5 new); full unfiltered browser suite 1146/1147 (the 1 remaining failure is `typography.test.ts` flagging a pre-existing `app.css` hardcoded font-size, confirmed present on an unmodified checkout, not touched here). Full reasoning and verification counts in decision 0490. **Decision 0489 (Reassign) confirmed pushed and deployed** — `origin/main` fetched directly reads `29b7d6e`, which is this decision's own commit, and the operator confirmed with “deployed and pushed and migrations applied.” Second of the five-step Coding-pilot sequence agreed after decision 0487 — handing a task to a named colleague, not just releasing it back to the pool. **No new permission** — reuses Release's own two-tier standing exactly: a person's own claim needs nothing extra, `AP.TaskManage` is the override for someone else's claim or an unclaimed task; `reassignStanding()` factors this out of `task-route.ts` so the candidates route and the action route can never silently disagree about who is allowed to act. **The target must independently hold the task's own `required_permission`**, not just team membership — decision 0010's own separation of the two facts about a person applies here too; a new `GET /tasks/:id/reassign-candidates` computes the eligible set server-side (team member ∩ permission holder, excluding the current claimant), the same "server decides, client doesn't infer" discipline `task.actions` itself already follows. **A dedicated `.backdrop`/`.popout` picker**, not the generic comment-and-OK/Cancel modal, which is still its own later, separate decision in the sequence. **`task_action_events` widened, not a new table** (migration `0084`): the `action` CHECK now includes `'reassign'`, plus a nullable `target_user_id` column, rebuilt the same way migration `0033` widened `tasks.status`, exactly the widening decision 0488 said this table was built to accept later. Tests: `vf-app` `task-route.test.ts` 45/45 (18 new), `task-list-route.test.ts` 62/62 (one updated for the intended widening), `activity-route.test.ts` 26/26 (1 new), `index.test.ts` in full 189/189 (4 new, one bug found and fixed along the way — a `.first()` query missing an `action = 'reassign'` filter picked up an earlier claim row instead). `vf-ui`: proxy allowlist `index.test.ts` 61/61 (2 new routes), browser `viewer.test.ts` 214/214 (5 new; the same pre-existing, unrelated `/api/documents/inv-1/collaborators` unhandled-rejection warnings scale from 235 to 240 in exact proportion, confirmed not a regression by running the unmodified file). Both migration chains (`vf-app` 84, `vf-licence` 169) replay clean. Full reasoning and verification counts in decision 0489. **Decision 0488 (Timeline/Chat audit entries for task actions) confirmed pushed and deployed** — `origin/main` fetched directly reads `010f277`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed, and migrations applied."* First of the five-step Coding-pilot sequence agreed after decision 0487, built from that decision's own deferred ask: every button/action taken should update the Timeline/Chat with the icon of the action, the actor's comment, their name, and a timestamp. **One new table, not five** — `task_action_events` (migration `0083`), scoped to exactly `claim`/`release`, the only two actions with no other durable trace (both can cycle on the same task, and `tasks.claimed_by`/`claimed_at` only ever shows the latest cycle; release leaves no trace at all today). Complete, Return, Return To Supplier, and Discard are each already fully recorded on `tasks` itself (`completed_by`/`completed_at`, `ended_by`/`ended_at`/`end_reason`/`status`/`returned_to_stage_id`) and derive their Timeline entries read-time in `activity-route.ts` (`taskActionEvents`, `taskEndedEvents`), respecting that file's own stated "no duplicate storage" principle — the same fault decisions 0236/0264 already fixed once. The `cancelled` siblings a return produces are explicitly excluded from the derivation, since a task moot because a colleague returned the document is not that colleague's own action. Both routes now accept an optional `comment` in the request body, parsed leniently (a missing or unparsable body is not an error, since every existing caller posts none today) — the column exists ready for the generic comment-and-OK/Cancel modal, itself a later, separate step in the agreed sequence, to fill in for every action once it exists. `workers/vf-ui/public/activity.js` renders the new unified `"action_taken"` kind with the real action icon (`icons.js`'s own `claim`/`release`/`return`/`return_to_supplier`/`discard` shapes) rather than the generic system-line dot, plus the comment underneath when one exists. New strings in `workers/vf-licence` migration `0168`. `vf-app`: `activity-route.test.ts` 25/25 (8 new), `task-route.test.ts`+`task-list-route.test.ts`+`invoice-facts-route.test.ts` 108/108 unaffected, `index.test.ts` 185/185 (5 new, including a full claim→release→claim cycle through the real router and the real activity endpoint). `vf-ui`: proxy allowlist `index.test.ts` 61/61 unchanged (no new route), browser `viewer.test.ts` 209/209 (5 new; the same 235 pre-existing, unrelated unhandled-rejection warnings already documented in decision 0487's own verification). Both migration chains (`vf-app` 83, `vf-licence` 168) replay clean. Full reasoning and verification counts in decision 0488. **Decision 0487 (Complete can recheck the rule that raised the task) confirmed pushed and deployed** — `origin/main` fetched directly reads `010f277`, which contains this decision's own commit `46f29bc`, and the operator confirmed with *"deployed and pushed, and migrations applied."* Asked live, about the Coding queue: clicking Complete should confirm the condition that raised the task was actually resolved, by re-firing that rule against live facts. Mid-conversation the operator described a much larger ask — a real per-stage action vocabulary (not today's seven generic `TaskAction`s), a comment-and-OK/Cancel modal per action with action-specific extra fields (a user picker for Reassign, a conditional manual-approver picker for Route To Approver, a reason/email dropdown pair for Return To Seller), and a Timeline/Chat audit entry on every action — of which this decision's rule re-check is one piece, the "upon clicking OK... Rule validation needs to execute" part specifically; the rest is deliberately not attempted here. **Built as a new table, `stage_actions` (migration `0082`) — one row per (stage, action), not a column on `process_stages`** — because the larger ask means more than one action will eventually want its own configured behaviour, which a column-per-action shape would mean a fresh migration for every one; sparse and off by default, so nothing is enforced until an operator opts a stage in. The re-check resolves the exact rule VERSION that fired (via `stage_visit_steps`, not whichever version is active now) against always-live facts (`loadLiveInvoiceFacts`, a new shared helper in `invoice-facts-route.ts` that also closed a duplication — the same facts-load sequence had already been written out twice in `index.ts`), and fails open — never blocks — whenever the exact thing to re-check cannot be confidently pinned down (a `system_reason` task, no stage visit behind it, no matching `stage_visit_steps` row, a non-invoice subject). The pre-check runs in `index.ts` ahead of `handleCompleteTask`'s own call, not inside it — the same import-direction discipline the existing completion cascade already follows — and refuses with 409 `{reason: "rule_still_fires", ruleName}` rather than completing; the task simply stays open and claimed, which is the operator's own "reasserted and available again" fallback, achieved by not completing it rather than new machinery. The refusal reuses the existing "here because" banner (decision 0478) — it flashes (`.reasonline-flash`, `--bg-warning`) rather than introducing a new UI pattern, the operator's own stated preference. New Stage Restrictions tab toggle ("Recheck the rule that raised this task before Complete succeeds," `vf-licence` migration `0167`, en/de), new `PUT /processes/stages/:id/actions/:action` route added to `vf-ui`'s proxy allowlist in the same change (decision 0484's own lesson). `vf-app`, `vf-ui`, and `vf-licence` all touched. Verified: `workers/vf-app`'s new `stage-actions-route.test.ts` **9/9**; `index.test.ts` in full **180/180** (2 new); `process-route.test.ts` **68/68** (2 new); `task-route.test.ts` + `task-list-route.test.ts` + `invoice-facts-route.test.ts` together **108/108**, untouched; `tsc --noEmit` clean on every touched file. `workers/vf-licence`'s `string-coverage.test.ts` + `ui-strings.test.ts` together **32/32**. `workers/vf-ui`'s `index.test.ts` (proxy allowlist) **61/61**; `ap-setup.test.ts` (browser) **49/49**; `viewer.test.ts` (browser) **204/204** (the same 235 unhandled-rejection warnings about an unstubbed collaborators fetch present identically on unmodified `main`, confirmed via `git stash`, entirely unrelated). Full reasoning and verification counts in decision 0487. **Decision 0486 (`POST /invoices/:id/key` never checked the claim) is confirmed pushed and deployed** — `origin/main` fetched directly reads `bcf0270`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed."* Reported live: Account Coding on a non-PO invoice in the Coding queue saved successfully although the item had not been claimed. This is the exact gap decision 0403 flagged but explicitly left unverified: that decision closed the client-side claim check for the line table (`cell()` joining `!canEditAnything`), and its own "What is not built" section said whether the save route itself refused an unclaimed edit sent directly "was not investigated as part of this decision." It did not — `handleKeyInvoiceFields` (`key-fields-route.ts`), the one route every keyed value saves through (including the Coding pop-out's own three fields), checked only permission scope (`AP.Validate`/`AP.Code`) and stage-configured field visibility, never task ownership. A working claim concept already existed elsewhere (`handleCompleteTask`'s own inline `task.owner_user_id`/`task.claimed_by` check, `task-route.ts`) — this route was simply never wired into it. Fixed on both sides: `handleKeyInvoiceFields` now refuses (403, `reason: "not_claimed"`) when an open task exists at the invoice's current stage and does not belong to the caller — scoped to when a task exists, not requiring one, so every pre-existing no-task test case (this route was never task-gated to begin with, only field- and stage-gated) is unchanged. `openLineCodingPopout` (`viewer.js`) now joins `!canEditAnything`, the same join `cell()` already makes for the line table proper. `vf-app` and `vf-ui` touched — no new migration, no `vf-licence` change. Verified: `test/key-fields.test.ts` **66/66** (7 new, 59 pre-existing unaffected); `keying-respects-the-stage.test.ts` **11/11**; `task-route.test.ts` + `task-list-route.test.ts` **90/90**, untouched. `vf-ui`'s Coding pop-out describe block **32/32** (6 new); full `viewer.test.ts` **204/204**; full browser suite **1123/1124** (the one failure a pre-existing, unrelated `document-window.test.ts` rejection, confirmed via `git stash`); worker suite **74/74**, untouched (no route or proxy-allowlist change needed). Full reasoning and verification counts in decision 0486. **Decision 0485 (which stages the Stage Restrictions screen even offers Account Coding checkboxes for) is confirmed pushed and deployed** — `origin/main` fetched directly reads `ce225f6`, matching this session's own commit exactly, and the operator confirmed with *"deployed."* Reported live, testing 0484's deploy: Intake and Payment Eligible showed an Account Coding checkbox despite neither stage ever having a person key a line — *"makes me wonder whether the default case should be off, or perhaps we limit the stages where this can be configured in the screen?"* Two options weighed: flipping the field-visibility default to off (rejected — the restrict-only invariant exists precisely so a screen never has to grant `edit` before it can configure anything, and it would silently change every existing stage's behaviour) vs. an explicit per-stage flag (chosen via `AskUserQuestion`) over an inferred heuristic from `rule_set_id`/`required_permission` (neither reliably signals "no person can ever key a line here," and guessing wrong risks hiding a stage that genuinely needs configuring, such as Approval or AP Review — the operator's own business-reason case). Built: `process_stages.offer_field_restrictions` (migration `0081`, default 1 — nothing visible today disappears on deploy), a new `handleSetStageOffersFieldRestrictions` route (`field-visibility-route.ts`/`index.ts`) mirroring the existing `handleSetStageReadOnly`, surfaced on `StageDetail` (`process-route.ts`), and the Stage Restrictions tab now branches each panel between the existing three checkboxes and a "not configurable here" explanation, with its own toggle either way. **The `vf-ui` proxy allowlist entry was added in this same change** — decision 0484's own lesson, applied before shipping rather than after a live report this time. **A pre-existing, unrelated gap found and fixed alongside this one**: `vf-licence/test/setup.ts` had never applied migrations 0163–0165 in its test schema, invisibly, because `string-coverage.test.ts`'s own hand-kept key list never had decision 0483's new strings added either — both fixed, and decision 0483's eight keys plus this decision's two (`vf-licence` migration `0166`) are now in that list. `vf-app`, `vf-ui`, and `vf-licence` all touched — no change to any existing stage's configured restrictions. Verified: `workers/vf-app`'s new `stage-offers-field-restrictions.test.ts` **6/6**, run together with `process-route.test.ts`/`field-visibility.test.ts`/`read-only-stage.test.ts` — all green; `tsc --noEmit` shows no new errors in any touched file. `workers/vf-ui` worker suite **74/74**; browser suite **1117/1118** (the one failure a pre-existing, unrelated `document-window.test.ts` rejection, confirmed via `git stash` to be present identically on unmodified `main`). `workers/vf-licence`'s `string-coverage.test.ts` + `ui-strings.test.ts` together **32/32** (the full `vf-licence` suite did not finish inside this session's time budget, matching this repo's own established pattern for that worker). **Still to do, operator side, once deployed: turn the new toggle off for Intake and Payment Eligible, and leave Approval and AP Review offered** — restricting Account Coding there was named as a real, wanted business decision. Full reasoning and verification counts in decision 0485. **Decision 0484 (the Stage Restrictions tab's own save route was never added to `vf-ui`'s proxy allowlist) is confirmed pushed and deployed** — `origin/main` fetched directly reads `d16ff60`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* Reported live, testing 0483: unchecking any Account Coding checkbox showed "not found" at the bottom of the screen. **A real bug, and a named recurring class of bug this project has caught many times before** (decisions 0212, 0319, 0324, 0418–0423, 0452, 0472, 0476, each doc-commented in place as a "found again" instance): `vf-ui` proxies `/api/*` to `vf-app` through an explicit allowlist, deliberately not a general forwarder, and `PUT /processes/stages/:id/field-visibility` — real and tested in `vf-app` since decision 0143/0196 — was never added to it, so every save from the new tab 404'd through the proxy's own fallback before `vf-app` ever saw the request. `GET /field-visibility` was already listed, which is why the tab rendered correctly and only the save failed. Missed for an ordinary reason: neither the backend tests (call the handler directly) nor the frontend browser tests (stub `fetch` itself) exercise `vf-ui`'s own routing worker end to end — only `index.test.ts`'s own `CALLED_BY_A_SCREEN` list does, and the new route wasn't added there either. Fixed: the pattern added to `PROXIED_TO_INSTANCE` in `workers/vf-ui/src/index.ts`, and the route added to `CALLED_BY_A_SCREEN` alongside it. `vf-ui` only — no `vf-app`/`vf-licence` change, no new migration. Verified: `test/index.test.ts` **61/61** (1 new); full `vf-ui` Worker suite **74/74**. Full reasoning and verification counts in decision 0484. **Decision 0483 (a Stage Restrictions tab in AP Setup, letting Account Coding be restricted per stage) is confirmed pushed and deployed** — `origin/main` fetched directly reads `abada4f`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed, applied migrations."* Reported live: Account Coding editable on the Validation stage, unexpectedly — "Coding should only happen in the Coding stage." The operator's own follow-up gave the real business reason: *"I would definitely need the restriction to prevent coding from validation. That is a business level activity that should be done only by AP team people... a company may wish to outsource, the document capture and data entry part of the process. This would mean that coding would need to be turned off at validation."* **Nothing here needed inventing** — `coding.project`/`coding.commodity_code`/`coding.gl_code` are ordinary vocabulary fields on the same customer/stage field-visibility system every other field uses, and the route to restrict one per stage (`PUT /processes/stages/:id/field-visibility`) has existed since decision 0143/0196, fully tested, with no screen ever built on top of it — AP Setup's own file comments already named that gap directly. Built: a fourth AP Setup tab, Stage Restrictions — one panel per stage, three auto-saving checkboxes for Account Coding, reading the stage's complete current restriction set fresh at save time so a restriction this tab does not render a checkbox for (set the raw way, or by a future screen) is never silently undone. `handleFieldVisibility` (`field-visibility-route.ts`) gained an opt-in `includeHidden` parameter, defaulting false, so this admin screen can see a field it is about to hide — every existing caller (the invoice viewer, `compose.js`, `rule.js`) unaffected. **A second piece of the same conversation — a stage optionally also evaluating another stage's own rule set, for matching-style discrepancy checks in Validation — was deliberately not built here**: agreed design (a small optional second `rule_set_id` per stage), but it touches the core rule-evaluation path and rules carry no category to filter by, a materially larger piece of work than a field-visibility checkbox. Tracked as its own follow-up decision. `vf-app`, `vf-ui`, and `vf-licence` (new migration `0165`, 8 keys × en/de) — no schema migration, reuses existing tables end to end. Verified: `test/field-visibility.test.ts` **29/29** (3 new); batch with `key-fields.test.ts` + `load-suppliers.test.ts` **230/230**; `vf-licence`'s `string-coverage.test.ts` **10/10**; `vf-ui`'s `ap-setup.test.ts` **43/43** (9 new); full `vf-ui` browser suite **1111/1112** (the one failure — an unrelated `document-window.test.ts` unhandled rejection — confirmed pre-existing via `git stash`). **Nothing in this decision changes any tenant's live data by itself — this session has no database access. Still to do, operator side, once deployed: in AP Setup → Stage Restrictions, uncheck all three Account Coding fields for the Validation stage — that is the actual fix for what was reported.** Full reasoning and verification counts in decision 0483. **Decision 0482 (the Seller card's VAT number, address, and postcode — never populated even when the data exists) is confirmed pushed and deployed** — `origin/main` fetched directly reads `50dd394`, matching this session's own commit exactly, and the operator confirmed with *"pushed and edployed."* Reported live: "the seller VAT no, or the address and postcode are never populated in the supplier card... the buyer card seems okay." **A real, pre-existing bug, predating this session by two weeks**: `handleGetInvoice`'s (`invoice-facts-route.ts`) `supplier:` response spread the raw D1 row straight through under its own SQL column names (`vat_id`, `address_line`, `postal_code`, `erp_identifier`, `is_pay_site`, `on_hold`, `hold_reason`), while `sellerPanel()` (`viewer.js`) has always read camelCase — the same explicit mapping `buyer` a few lines above has always had. `city`/`country` happened to match either way, which is exactly why only VAT/address/postcode read as missing while the buyer card looked fine; the ERP/site sub-line, the pay-site tag, and the on-hold banner were silently affected too, though not reported. **The existing tests had enshrined the bug as the contract** — three assertions in `load-suppliers.test.ts` checked the wrong, snake_case field names as if correct, exactly why this went uncaught; corrected alongside the fix, with the first extended to assert `vatId`/`addressLine`/`postalCode` directly. `vf-app` only — no `vf-ui`/`vf-licence` change, no new migration, no new string. Verified: `test/load-suppliers.test.ts` + `test/key-fields.test.ts` + `test/invoice-facts-route.test.ts` **219/219**; every file touching `invoice-facts-route.ts` together **376/376**. Full reasoning and verification counts in decision 0482. **Decision 0481 (New Seller cosmetics — icon and placement beside Change Seller, Save/Close moved to the pop-out's own top right with a Save icon, and a real Close bug fixed) is confirmed pushed and deployed** — `origin/main` fetched directly reads `0553b7c`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* Reported live right after testing decision 0480's own invoices. **A genuine bug, not just unstyled**: the pop-out's Close button had no `onclick` at all, which `actionLink()` treats as "nothing to do" and marks `disabled` — a handler patched on afterwards by querying the DOM never ran, since a disabled button never dispatches a click regardless. Wired directly now. Both cosmetic requests built from conventions already in this codebase: `suppliers.js`'s own two-button card header (decision 0300) for Change Seller/New Seller together, and `access.js`/`coding-lists.js`'s own pop-out `.cardhead` for Save/Close top right. New `newseller` icon (`icons.js`), same shape as `newsupplier`/`newperson` under its own key, decision 0328's established reuse convention. `vf-ui` only — no `vf-app`/`vf-licence` change, no new migration, no new string. `test-browser/viewer.test.ts` **198/198** (3 new, in the existing New Seller block), full browser suite **1102/1103** (the one failure pre-existing/unrelated, `typography.test.ts`'s hardcoded-`10px` finding). Full reasoning and verification counts in decision 0481. **Decision 0480 (may not leave the last checkpoint without a supplier the ERP knows, plus the New Seller form) is confirmed pushed and deployed** — `origin/main` fetched directly reads `6b18b85`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed, migrations also applied."* The operator's own request in full: identify Seller/Buyer at validation; if the seller cannot be found on a new Non-PO invoice, offer a New Seller form (Company Name, Tax ID, e-mail, Address); the invoice may not reach the ERP until a real ERP identifier exists for that supplier; a PO invoice with no supplier matched at all should never happen and gets no such form. Two design questions settled via AskUserQuestion before any code: the gate is hardcoded, not a tenant-configurable rule; a PO-invoice-with-no-supplier is blocked distinctly with no self-service. **A significant reversal along the way, caught only by running the real test suite**: the gate's first draft read `invoice_headers.supplier_id`/`suppliers.erp_identifier` live via a JOIN, reasoning a live read is more robust — proven backwards, because decision 0434's own architecture writes that durable column only *after* the same cascading `visitCurrentStage` call that computes the facts returns, so a live read always sees a stale value on a clean invoice's first capture and would have blocked every ordinary invoice in production. Fixed to read `facts["supplier.matched"]`/`facts["supplier.awaitingErp"]` instead, the same facts every earlier-stage rule already evaluates against. Also found and fixed along the way: `handleSetInvoiceSupplier` (decision 0222's attach-by-hand route) previously refreshed `supplier.matched` alone, leaving `.awaitingErp` and every other supplier-derived fact at old, often-wrong defaults. `workers/vf-app`: new migration `0080` (`tasks.system_reason`, three mutually-exclusive values), `erpReleaseGuard()` in `workflow-engine.ts`, `load-suppliers.ts`'s fact-refresh fix, `currentOpenTaskReason` (`invoice-facts-route.ts`) extended with a `UNION ALL` covering this new kind of task. `workers/vf-ui`: New Seller button/form and the PO-anomaly warning in `viewer.js`'s `sellerPanel()`, `reasonLinePanel()` extended for the new system-reason banner text. `workers/vf-licence`: new migration `0164` (10 keys × en/de). Verified: `test/workflow-engine.test.ts` 68/68 (8 new), `test/load-suppliers.test.ts` 142/142 (2 new), `test/key-fields.test.ts` 59/59 (3 new, 1 updated), a further ~10 pre-existing tests updated across 4 files for the new invariant, batch run across 14 files together **618/619** (the one failure pre-existing/unrelated, `stage-permissions.test.ts`'s missing `AP.Manager`, confirmed via `git stash`). `vf-ui` `test-browser/viewer.test.ts` **195/195** (7 new), full browser suite **1099/1100** (the one failure pre-existing/unrelated, `typography.test.ts`'s hardcoded-`10px` finding in `.collabchip .activityavatar` from decision 0470, confirmed via `git stash`). Full reasoning and verification counts in decision 0480. **Decision 0479 (`.c-process` shares its stretched height, instead of every panel claiming it whole) is confirmed pushed and deployed** — `origin/main` fetched directly reads `f1a81b1`, matching this session's own commit exactly, and the operator confirmed with *"pushed and deployed successfully."* A real bug, decision 0478's own doing: its new `reasonLinePanel()` made `#viewer .columns.docpoppedout .c-process` commonly hold two or three panels at once, and the old `height: 100%` rule (decisions 0392/0393, written for exactly one panel) gave each one the row's full height — stacked, overflowing the row and reading as the process timeline overlaying the Seller/Buyer cards beneath it, reported live against a screenshot. Fixed with a flex column on `.c-process` (banner panels at their own natural height, only the last panel — `progressRow()`, when present — growing to fill the rest), plus a related, independently pre-existing gap found in the same screenshot: `.c-header` never stretched to match a taller `.c-parties`, only the reverse (decision 0393's own one-directional fix) — now symmetric. Verified visually, not just argued about, with a before/after Playwright screenshot comparison across the one-, two-, and three-panel cases. `vf-ui` browser suite **1092/1093** (the one failure pre-existing and unrelated — the same `typography.test.ts` finding decision 0478 already documented), 2 new tests in `viewer.test.ts` asserting the actual CSS rule text. `vf-ui` Worker unchanged at 74/74. No `vf-app`/`vf-licence` change, no new migration. Full reasoning and verification counts in decision 0479. |
| vf-admin deployed | `8e27a34` · `https://admin.vibefinance-ai.com` · behind Cloudflare Access |
| vf-app deployed | `ca1de60` confirmed — decisions 0429 (agreed payment means, a supplier-record placeholder), 0430 (Talk to an AP Expert, Screen 6) with all eight of its own addenda, 0431 (Executive IQ's remaining four metrics), 0432 with its own addendum, 0433 (org-ranked supplier search), 0434 (org/supplier facts reach the first stage visit), 0435 (a stage visit error is recorded, not swallowed), and 0436 (vf-ui only — the nav's own content scrolls), all confirmed. Decision 0439 (Approval Hierarchy — schema and resolver, vf-app only) is confirmed pushed and deployed, per the operator's own report. **Decision 0440's own write API (`approval-config-route.ts`) is now also confirmed pushed and deployed, per the operator's own report.** **Decision 0444's own `coding-list-route.ts` (the generic Account Coding CRUD) plus its `GET /org/cost-centres` and `ledger-route.ts` extensions are confirmed pushed and deployed, per the operator's own report.** **Decision 0445's own `coding-list-csv-route.ts` (CSV Template and Load for the four applicable coding lists) is confirmed pushed and deployed, per the operator's own report.** **Decision 0446's own search/pagination additions to `coding-list-route.ts` and `ledger-route.ts` (plus the `index.ts` query-param wiring) are confirmed pushed and deployed, per the operator's own report.** **Decision 0448's own search/pagination additions to `documents-route.ts` (66/66, plus `ap-assistant.test.ts`'s own 68/68 confirming `runInvoiceSearch()`'s backward compatibility) are confirmed pushed and deployed, per the operator's own report — *"deployed and pushed - looks great I have checked."* No `index.ts` change needed: `page`/`pageSize` already flowed through its existing query-param passthrough.** **Decision 0449's own rewritten `task-list-route.ts` (search and real pagination pushed into SQL, plus a genuine SQL three-valued-logic NULL-propagation bug found and fixed along the way in the ownership-count query) and `index.ts`'s `/tasks` route wiring (`q`/`page`/`pageSize`, the same shape `/documents` already reads) are confirmed pushed and deployed, per the operator's own report — *"deployed and pushed"*, then *"tested as working."*** **Decision 0452's own `approval-hierarchy.ts` (the new `resolveApprovalTargets`/`resolveCostObjects` generalization), `workflow-engine.ts` (multi-task-per-line spawning), `coding-list-route.ts` (`approvalLimit` on the three greenfield lists), and `approval-config-route.ts`/`index.ts` (the new `PUT /approval-config/cost-object-dimensions` route) are confirmed pushed and deployed — `origin/main` fetched directly reads `7e2d4ed`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-app`.** **Decision 0453's own `enforce.ts` (the new `requireAnyPermission`), `index.ts` (`GET /org/cost-centres` and `GET /coding-lists/:type` now `Admin.Configure` OR `AP.Validate`, plus `filter.<type>` query-param wiring), `coding-list-route.ts`, and `ledger-route.ts` (both gaining the new `filters` narrowing parameter) are confirmed pushed and deployed — `origin/main` fetched directly reads `ccbbc82`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-app`.** **Decision 0454's own `workflow-engine.ts` (`onTaskCompleted` now reports where its cascade stopped instead of silently parking) and `index.ts` (the new `followUpAfterTaskCompletion`, loading real facts and stored lines and making the genuine `visitCurrentStage` call a rule-bearing stage always needed) and `invoice-facts-route.ts` (the new `loadStoredInvoiceLines`) are confirmed pushed and deployed — `origin/main` fetched directly reads `93b4568`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-app`.** **Decisions 0455 (`index.ts`'s `GET /org/cost-centres` and `GET /coding-lists/:type` widened once more, from `Admin.Configure OR AP.Validate` to also accept `AP.Code`), 0456 (six more routes on the Coding task path — `GET /invoices/:id`, `POST /invoices/:id/document-url`, `GET /invoices/:id/pages`, `POST /invoices/:id/pages/:n/document-url`, `GET /invoices/:id/progress`, and `POST /invoices/:id/key` — all widened to also accept `AP.Code`), and 0457 (the new `coding-suggestions.ts` module and `GET /invoices/:id/coding-suggestions` route) are all confirmed pushed and deployed — `origin/main` fetched directly reads `2b4c520`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-app`.** **Decision 0463's own new `POSSIBLE_DUPLICATE_THRESHOLD` constant in `invoice-history.ts`, imported by `dashboard-route.ts`, `documents-route.ts`, and `fraud-duplicates-route.ts` in place of each stating its own `0.5`, is confirmed pushed and deployed — `origin/main` fetched directly reads `e6f4604`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."*** |
| vf-licence deployed | `ca1de60` per the operator's own reports; migrations `0140` through `0144` all applied — `0144` is decision 0435's own banner-label string. Decision 0436 added none. **Migration `0145` (decision 0440's own AP Setup strings, 29 keys × en/de) is now also confirmed applied, per the operator's own report.** **Migration `0147` (decision 0444's own Account Coding strings, 22 keys × en/de) is applied and confirmed live — see the `vf-licence-poc` migrations row.** **Migration `0148` (decision 0445's own CSV Template/Load strings, 16 keys × en/de) is confirmed pushed, per the operator's own report — not yet separately applied. See "What was found, not built" in `docs/decisions/0445-csv-template-and-load-for-coding-lists.md` for a genuine, unrelated test-harness bug this migration's own SQL surfaced and was fixed alongside.** **Migration `0149` (decision 0446's own two search/pagination strings — `apsetup.codingsearchplaceholder`, `apsetup.codingnomatches` — 2 keys × en/de) is confirmed pushed, per the operator's own report — not yet separately applied.** **Migration `0150` (decision 0447 — value-only UPDATEs to `roles.units`, `apsetup.codingtab.companycode`, and `apsetup.codingcompanycodesub`, 3 keys × en/de, 6 rows) is confirmed pushed, per the operator's own report — not yet separately applied.** **Migration `0151` (decision 0449's own three search/pagination strings — `tasks.searchhint`, `tasks.nomatch`, `tasks.countsline` — 3 keys × en/de) is written and replay-tested (151 migrations, all assertions held) and confirmed pushed as part of decision 0449's own confirmed commit — the operator's own report, *"deployed and pushed"*, then *"tested as working"* — but not yet separately applied.** **Migration `0152` (decision 0451's own three new field labels — `field.coding.project`, `field.coding.commodity_code`, `field.coding.gl_code`, 3 keys × en/de) is confirmed pushed, deployed, and applied — the operator's own report, *"pushed and deployed and sql applied,"* all three in one, unlike most migrations before it in this table.** **Migration `0153` (decision 0452's own four new Cost-Object Priority panel strings — `apsetup.costobjectpriority`, `.costobjectprioritysub`, `.costobjectenable`, `.costobjectsavefailed`, 4 keys × en/de) is confirmed pushed, deployed, and applied — `origin/main` fetched directly reads `7e2d4ed`, `wrangler deploy` confirmed run for `vf-licence`, and the operator's own `apply_migrations.py --remote --database vf-licence-poc --migrations-dir workers/vf-licence/migrations` output confirms it applied (`checksum 71fd6f98e007...`).** **Migration `0154` (decision 0453's own seven new invoice-line Coding pop-out strings — `action.coding`, `viewer.coding.heading`/`.searchhint`/`.nomatches`/`.searchfailed`/`.clear`/`.noteditable`, 7 keys × en/de) is confirmed pushed, deployed, and applied — `origin/main` fetched directly reads `ccbbc82`, matching this session's own commit exactly, `wrangler deploy` confirmed run for `vf-licence`, and the operator confirmed the migration separately applied to `vf-licence-poc`.** **Migration `0155` (decision 0457's own one new string, `viewer.coding.suggested`, en/de) is confirmed pushed, deployed, and applied — the operator's own report, *"pushed and deployed,"* with the migration's own separate `apply_migrations.py --remote` run also confirmed. See the `vf-licence-poc` migrations row.** **Migration `0156` (decision 0458's own one new string, `viewer.coding.resultsfor`, en/de) is confirmed pushed, deployed, and applied — the operator's own report, *"deployed and pushed,"* with the migration's own separate `apply_migrations.py --remote` run also confirmed. See the `vf-licence-poc` migrations row.** **Migration `0157` (decision 0459's own one new string, `viewer.coding.nomatchesscoped`, en/de) is confirmed pushed, deployed, and applied — the operator's own report, *"deployed and pushed,"* with the migration's own separate `apply_migrations.py --remote` run also confirmed. See the `vf-licence-poc` migrations row.** |
| vf-ui deployed | `ca1de60` · `https://app.vibefinance-ai.com` — operator's own reports, confirmed directly: *"deployed and pushed"* against the icon buttons live, then, with a screenshot, *"the icons are a little lower or the text box is higher. They seem a little un-aligned"*, then *"pushed and deployed"* again confirming the alignment-fix addendum live, then *"deployed and pushed"* confirming decision 0433's own org-ranked supplier search live, then *"pushed and deployed - this seems to have worked"* confirming decision 0434 live, then *"pushed and deployed"* again confirming decision 0435 live, then *"pushed and deployed"* again confirming decision 0436 live, then *"deployed and pushed - works great"* confirming decisions 0437 and 0438 live together, then *"pushed and deployed"* again confirming decision 0440's own `ap-setup.js` (plus the exported `currencyPicker` and the new `apsetup` nav icon) live. **Decision 0440's own screen then failed live** — *"AP Setup could not be loaded"* — root-caused to `vf-ui`'s own `PROXIED_TO_INSTANCE` allowlist never learning about the five new `/approval-config` routes; fixed as decision 0441, confirmed deployed with *"deployed and pushed."* **The same message then recurred for a second, unrelated reason** — see the `vf-app-poc` migrations row — now also resolved, confirmed by the operator's own *"that worked - thank you."* **Decision 0442's own reorder-and-search UI change is confirmed deployed, *"deployed and pushed."*** The operator then reported the new search-hint text showing as raw, untranslated keys, resolved once migration `0146` was applied — confirmed with *"yes, I see the proper text hints now."* **Decision 0443's own icon-button change is confirmed deployed too — *"pushed and deployed - thank you."*** **Decision 0444's own Account Coding tab (`coding-lists.js`, wired into `ap-setup.js`, plus two new `PROXIED_TO_INSTANCE` route patterns and two long-pre-existing unproxied cost-centre routes fixed alongside them) is confirmed deployed, *"deployed and pushed."*** **Decision 0444's own screen then failed live, the identical report decision 0441 already made once** — *"AP Setup could not be loaded"* — but this time the proxy allowlist was correct (checked directly, not assumed): the operator's own browser console showed `overview 200, config 200, cost centres 500, project 500, commodity code 500, gl code 500` — exactly the four routes touching the four new tables migration `0076` creates, and only those four. Root-caused to `0076` never having been separately applied to `vf-app-poc`, the identical shape decision 0441's own second finding (migration `0075`) already was; resolved with no code change once the operator ran `apply_migrations.py --remote`, confirmed with *"that worked - thank you."* **Decision 0445's own `coding-lists.js` CSV loader panels, wired into all four applicable Account Coding sub-tabs, are confirmed deployed — *"deployed and pushed."*** **Decision 0446's own rewritten `coding-lists.js` (search + real pagination for Cost Centre, Project, Commodity Code, and General Ledger Code, plus the simplified `ap-setup.js` wiring) is confirmed deployed — *"deployed and pushed - looks good."*** Given the standing habit note below, the operator's own next step is to separately apply migration `0149` (see the `vf-licence-poc` migrations row) — deploying the code alone will not do it; until then, expect the new search box's placeholder and no-matches message to show as raw string keys rather than real text. **Decision 0447's own `coding-lists.js` change (Template/Load buttons reused from Purchase Orders' own strings) is confirmed deployed — *"deployed and pushed."*** Given the standing habit note above, the operator's own next step is to separately apply migration `0150` (see the `vf-licence-poc` migrations row) — deploying the code alone will not do it; until then, expect the AP Setup Template/Load buttons to keep reading "Template"/"Load" and the Access/AP Setup tabs to keep their old, misaligned names. **Decision 0448's own rewritten `documents.js` (real, server-side search and pagination for the Documents screen, the same treatment decision 0446 gave Account Coding — see `docs/decisions/0448-search-and-pagination-for-documents.md`) is confirmed deployed — *"deployed and pushed - looks great I have checked."*** No migration to apply — this decision needed none. **Decision 0449's own rewritten `tasks.js` (the same `load()`/`render()`/`reload()` split decision 0448 gave `documents.js`, real search and pagination, the decision-0359 stage-select-patch hack retired now that `render()` rebuilds the stage `<select>` fresh every time, plus a genuine `problem()` null-pointer bug found and fixed along the way) is confirmed deployed — *"deployed and pushed"*, then *"tested as working."*** Given the now well-established habit noted throughout this table, the operator's own next step is to separately apply migration `0151` (see the `vf-licence-poc` migrations row) — deploying the code alone will not do it; until then, expect the new Tasks search box's placeholder and no-matches message to show as raw string keys rather than real text. **Decision 0453's own `viewer.js` (the new Coding icon and per-line pop-out, `openLineCodingPopout`, `searchableEntryPicker`, and `fetchCodingEntries`), `icons.js` (the new `coding` glyph), and `app.css` (the widened line-table trailing column and the new `.lineactions`/`.codingsearch` rules) are confirmed pushed and deployed — `origin/main` fetched directly reads `ccbbc82`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-ui`.** **Decision 0457's own rewritten `openLineCodingPopout` (the new `fetchCodingSuggestions()` and the pre-fill/suggested-note behaviour) is confirmed pushed and deployed — `origin/main` fetched directly reads `2b4c520`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-ui`.** **Decision 0458's own further-rewritten `openLineCodingPopout` (the fixed-size `.popout.codingpopout`, the new `codingResultsController`, and `searchableEntryPicker`'s own rewrite to use it) and `app.css`'s new `.codingpopout`/`.codingresults`/`.codingresultslist` rules are confirmed pushed and deployed — `origin/main` fetched directly reads `1d8c423`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-ui`.** **Decision 0459's own `searchableEntryPicker` (new `preload`/`scopeNote` options), `openLineCodingPopout` (the Cost Centre auto-focus handle), and `app.css`'s `.codingresultslist` reverted to one column, are confirmed pushed and deployed — `origin/main` fetched directly reads `e02f187`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-ui`.** **Decision 0460's own further-rewritten `searchableEntryPicker` (`onfocus` now runs the same search `oninput` does, `codingResultsController`'s new `peek()` guarding the one race this reopened, and the two-character minimum removed) and `app.css`'s narrower `.codingsearch` plus the new `.codingcompanycode` rule are confirmed pushed and deployed — `origin/main` fetched directly reads `52e3d23`, matching this session's own commit exactly, and the operator confirmed `wrangler deploy` run for `vf-ui`. Live, the narrowing itself was reported not yet right (see the `origin/main` row above) — worked out as a mock-up first.** **Decision 0461's own rewritten `.popout.codingpopout` (`--codingfield-label-w`/`--codingfield-w` driving the width in one place instead of three) in `app.css` — no `viewer.js` change, CSS only — is confirmed pushed and deployed together with decision 0462 — first reported live as still wrong: *"way out of line again... it spills out of the page."*** **Decision 0462's own one-line fix — `box-sizing: border-box` added to `.popout.codingpopout .codingcompanycode` — is confirmed pushed and deployed — `origin/main` fetched directly reads `1b64da3`, matching this session's own commit exactly, and the operator confirmed with *"deployed and pushed."* `.readonly` (a `<div>`) never inherits `tokens.css`'s own `box-sizing: border-box`, stated only for `input`/`textarea`; a content-box `width: 100%` plus its own padding was rendering 22px past the column both 0460 and 0461 were aiming it at, the whole reason matching the search boxes' own `width` number twice never actually matched their pixel width.** |
| Domain | `vibefinance-ai.com` · **email intake receives real invoices** |
| `vf-app-poc` migrations | through `0075` applied and confirmed live — `0073` (decision 0429) is real schema; `0074` (decision 0430) is a documentation-only `ASSERT` restatement with no schema change, the same shape as `0071`; none of 0430's eight addenda needed a new `vf-app` migration; decision 0431 also needed none — its four new routes read existing tables only; decision 0433 also needed none — its ranking change reads the existing `org_unit_id` column only; decision 0434 also needed none — it changes when facts already computed reach the workflow engine, not the schema; decision 0435 also needed none — it writes a new fact through the existing `facts_json` column. **A tenant-data fix, not a migration**: the operator's own live Validation stage had `required_permission IS NULL` — the root cause behind decision 0435's own finding — fixed directly with `UPDATE process_stages SET required_permission = 'AP.Validate' WHERE id = 'validation'`. **This did not hold on the first attempt**: after decision 0435 deployed, a fresh test invoice hit the identical `requiredPermission "undefined"` error via the new `workflow.stageError` banner, and a direct re-check found `required_permission` back to `NULL` — code was traced end to end (`process-route.ts`'s stage-creation and draft/publish handlers, `field-visibility-route.ts`, `rules-list-route.ts`) and **nothing in the application ever writes this column**, so the revert's cause is unexplained, not a known bug. Re-run a second time with the `UPDATE` and a `SELECT` in the same statement batch, confirmed set to `AP.Validate` in that same round-trip, and then confirmed durable and working end-to-end by the operator submitting a genuinely fresh test invoice: it stopped at Validation, no error banner, and a task appeared with `required_permission = AP.Validate`. **If this reverts a third time**, suspect a second database bound to the same `vf-app-poc` name (check `wrangler d1 list` against `workers/vf-app/wrangler.toml`'s `database_id`) rather than re-tracing application code again. `0075` (decision 0439, Approval Hierarchy) is applied and confirmed live — the operator's own report, *"pushed and deployed"*. **Decision 0440 needed no new `vf-app` migration — its two new tables already existed from `0075`; this is a routes-and-UI-only decision.** **That "applied and confirmed live" claim for `0075` was wrong, found live during decision 0441's own follow-up.** Once decision 0441's own proxy fix deployed, `GET /api/approval-config` still failed — now a `500`, not a `404` — and a direct, operator-run, read-only query settled it: `npx wrangler d1 execute vf-app-poc --remote --command "SELECT mode, default_approver_user_id FROM org_approval_config WHERE id = 1"` returned `no such table: org_approval_config`. Migration `0075` had never actually applied to `vf-app-poc`, despite being reported and recorded as confirmed at the time decision 0439 shipped — nothing dishonest in that record, the code and the worker deploy were genuinely correct, but this project keeps applying a migration a separate, operator-run step from deploying a worker (`apply_migrations.py --remote`, never run by this session), and that step was missed for `0075` specifically. It also took this long to surface because nothing had genuinely reached `org_approval_config` live before — decision 0440's own screen never got past the proxy rejection to try. **Fixed with no code change**: the operator ran `python3 migrations/apply_migrations.py --remote` (default database `vf-app-poc`), which applied `0075` for real, and confirmed immediately after with *"that worked - thank you."* `0075` is now genuinely applied and confirmed live. **`0076` (decision 0444, the Account Coding schema — `coding_list_types`/`coding_list_type_filters`/`coding_list_entries`/`coding_list_entry_filters`) is written and replay-tested (`--replay-only`: 76 migrations, 170 standing invariants, all held).** **It was NOT applied when decision 0444 first deployed — the third instance this session of the exact same slip (`0075` above, `0146` below), and the operator's own live report — *"AP Setup could not be loaded"* — was identical to decision 0441's original one. Diagnosed this time from the browser console alone, without re-tracing any code**: `overview 200, config 200, cost centres 500, project 500, commodity code 500, gl code 500` — the two untouched routes read pre-existing tables and succeeded; the four new/extended ones (Cost Centre's own route now also joins `coding_list_entry_filters` for its new company-code filter) all read tables `0076` creates and all failed the same way. **Fixed with no code change**: the operator ran `python3 migrations/apply_migrations.py --remote` (default database `vf-app-poc`), confirmed immediately after with *"that worked - thank you."* `0076` is now genuinely applied and confirmed live.** **Decision 0445 needed no new `vf-app` migration — CSV Template and Load reads and writes through `coding_list_entries`/`cost_centres` and their existing filter tables, all of which `0076` already created; nothing new to apply here.** **Decision 0446 also needed no new `vf-app` migration — search and pagination read the same existing tables and columns through new query parameters only; nothing new to apply here either.** **Decision 0448 also needed no new `vf-app` migration, for the same reason — Documents' own search and pagination read `invoice_headers`, `inbound_email_events`, and the other pre-existing joined tables unchanged.** **`0077` (decision 0452, the Cost-Object schema — `approval_limit` on `coding_list_entries` plus the new `cost_object_dimensions` config table) is applied and confirmed live — the operator's own `apply_migrations.py --remote` output (default database `vf-app-poc`) confirms it applied (`checksum ebcafbc088a3...`), and `wrangler deploy` was confirmed run for `vf-app` too.** **Decision 0457 needed no new `vf-app` migration — `coding-suggestions.ts` reads the existing `keyed_fields`/`invoice_headers` tables only.** **Decision 0458 needed no `vf-app` change at all — a `vf-ui`/`vf-licence`-only layout decision.** **`0078` (decision 0469, Phase 1 of Matching Exceptions and Non-PO Approval routing — `buyer_user_id` on `purchase_orders`; `invoice_collaborators`; `org_matching_config` singleton; `org_approval_config.route_non_po_to_requester`; also restates the permission-vocabulary standing invariant with `Procurement.Collaborate`/`Procurement.Approve` added) is applied and confirmed live — replay-tested clean first (`--replay-only`: 78 migrations, 184 standing invariants, all held), then the operator confirmed with *"yes - sql is deployed."* Zero behaviour change from applying it: every supplier still gets its own already-configured tolerance or the new org default (itself `0` until set), and the Non-PO Approval toggle stays off by default and inert regardless of the toggle, since no route yet populates `invoice_collaborators`.** |
| `vf-licence-poc` migrations | through `0145` applied and confirmed live — the operator's own `apply_migrations.py --remote` run, `0144` is decision 0435's own banner-label string (`viewer.workflow.stageerror`, en/de). **`0145` (decision 0440's own AP Setup strings) is now applied and confirmed live too.** **`0146` (decision 0442's own override-list search strings) is now applied and confirmed live too.** Once 0442 deployed, the operator first reported the two new search-hint strings showing as raw, untranslated keys (`apsetup.supervisoroverridesearchhint`, `apsetup.limitoverridesearchhint`) rather than their real text — read as the same shape decision 0441's own second finding already was (a migration reported via code deploy, not yet separately applied), rather than a code bug, since both keys matched `ap-setup.js`'s own calls and `0146`'s own rows exactly. Named to the operator as the likely cause and the fix (`apply_migrations.py --remote --database vf-licence-poc --migrations-dir workers/vf-licence/migrations`) rather than assumed silently — confirmed correct once applied: *"yes, I see the proper text hints now."* **`0147` (decision 0444, Account Coding strings, 22 keys × en/de) is applied and confirmed live** (`apply_migrations.py --remote --database vf-licence-poc --migrations-dir workers/vf-licence/migrations`) — the operator confirmed real labels are showing, not raw keys, with *"yes labels are showing."* **`0148` (decision 0445, CSV Template/Load strings, 16 keys × en/de) is written and replay-tested (`--replay-only`: 148 migrations, 73 standing invariants, all held) and now confirmed deployed with the rest of decision 0445's code — but not yet separately applied. The operator's own next step is `apply_migrations.py --remote --database vf-licence-poc --migrations-dir workers/vf-licence/migrations`, per the now-three-times-repeated habit named below — expect the Account Coding CSV strings to show as raw keys, or the new routes to 500, until this runs.** **`0149` (decision 0446, the two new search/pagination strings — `apsetup.codingsearchplaceholder`, `apsetup.codingnomatches` — 2 keys × en/de) is written and confirmed via `test/setup.ts` + `string-coverage.test.ts` (both green), and confirmed pushed and deployed with the rest of decision 0446's code, per the operator's own report — but not yet separately applied. Expect the same raw-key symptom on the Account Coding search box and its no-matches message until this migration is separately applied.** **`0150` (decision 0447, value-only UPDATEs to `roles.units`, `apsetup.codingtab.companycode`, and `apsetup.codingcompanycodesub`, 3 keys × en/de, 6 rows) is confirmed pushed and deployed with the rest of decision 0447's code, per the operator's own report — but not yet separately applied. Until it runs, the Access "Org units" tab and AP Setup "Company code" tab keep their old, unaligned names.** **`0151` (decision 0449, the three new Tasks search/pagination strings — `tasks.searchhint`, `tasks.nomatch`, `tasks.countsline` — 3 keys × en/de) is written and confirmed via `test/setup.ts` + `string-coverage.test.ts` (both green), and confirmed pushed and deployed with the rest of decision 0449's code — but not yet separately applied. Expect the same raw-key symptom on the Tasks search box and its no-matches message until this migration is separately applied.** **`0152` (decision 0451, the three new Line Level Account Coding field labels — `field.coding.project`, `field.coding.commodity_code`, `field.coding.gl_code` — 3 keys × en/de) is applied and confirmed live — the operator's own report, *"pushed and deployed and sql applied,"* the first migration in this table confirmed applied in the same report as the deploy itself rather than as a separate later step.** **`0153` (decision 0452, the four new Cost-Object Priority panel strings — `apsetup.costobjectpriority`, `.costobjectprioritysub`, `.costobjectenable`, `.costobjectsavefailed` — 4 keys × en/de) is applied and confirmed live — `apply_migrations.py --remote --database vf-licence-poc --migrations-dir workers/vf-licence/migrations` output confirms it applied (`checksum 71fd6f98e007...`), reported directly by the operator.** **`0154` (decision 0453, the seven new invoice-line Coding pop-out strings — `action.coding`, `viewer.coding.heading`/`.searchhint`/`.nomatches`/`.searchfailed`/`.clear`/`.noteditable` — 7 keys × en/de) is applied and confirmed live — `origin/main` fetched directly reads `ccbbc82`, matching this session's own commit exactly, and the operator confirmed the migration separately applied to `vf-licence-poc`.** **`0155` (decision 0457, one new string — `viewer.coding.suggested` — 1 key × en/de) is applied and confirmed live — the operator's own report, *"pushed and deployed,"* with the separate `apply_migrations.py --remote` run against `vf-licence-poc` confirmed.** **`0156` (decision 0458, one new string — `viewer.coding.resultsfor` — 1 key × en/de) is applied and confirmed live — the operator's own report, *"deployed and pushed,"* with the separate `apply_migrations.py --remote` run against `vf-licence-poc` confirmed.** **`0157` (decision 0459, one new string — `viewer.coding.nomatchesscoped` — 1 key × en/de) is applied and confirmed live — the operator's own report, *"deployed and pushed,"* with the separate `apply_migrations.py --remote` run against `vf-licence-poc` confirmed.** **`0158` (decision 0470, the five new "Add person to conversation" strings — `activity.addperson`, `.addpersonsearch`, `.addpersonnomatches`, `.addpersonfailed`, `.collaborators` — 5 keys × en/de) is applied and confirmed live — the operator's own report, *"I see the add person in the chat now."*** **`0159` (decision 0471, one new string — `action.approve`, the Business Approver's own label for the existing Complete action — 1 key × en/de) is confirmed pushed and deployed as part of decision 0471's own commit — the operator's own report, *"deployed and pushed"* — but not yet separately applied. Until `apply_migrations.py --remote --database vf-licence-poc --migrations-dir workers/vf-licence/migrations` runs, expect a Business Approver's Approve button to show the raw key `action.approve` rather than real text.** **`0160` (decision 0472, the five new Matching tab strings — `apsetup.matchingsub`, `.amounttolerance`, `.quantitytolerance`, `.quantitymatchingenabled`, `.savematchingfailed` — 5 keys × en/de) is applied and confirmed live — the operator's own report, *"migrations have been applied."*** **`0161` (decision 0474, the six new standard-matching-rules strings — `apsetup.standardrules`, `.standardrulessub`, `.standardrulenotcreated`, `.standardrulesuggested`, `.standardrulepending`, `.standardrulesavefailed` — 6 keys × en/de) is applied and confirmed live — the operator's own report, *"pushed and deployed, migrations applied,"* both in one report.** **`0162` (decision 0476, the two new "remove collaborator" strings — `activity.removeperson`, `activity.removepersonfailed` — 2 keys × en/de) is applied and confirmed live — the operator's own report, *"pushed and deployed, migrations applied,"* both in one report.** |
| Tests | vf-admin 9 · vf-app **2618** (2587 + 31 new — 17 in `coding-list-route.test.ts`, 14 in `accounting-frame.test.ts`, decision 0446, confirmed by one unfiltered whole-suite run — 113 files, all green) · vf-licence **320** (unchanged in count — decision 0446's own migration 0149 adds rows to the existing `ui_strings` table, not a new test file; confirmed by one unfiltered whole-suite run — 21 files, all green) · vf-ui 74 Worker (unchanged since 0440 — no proxy/route change since, confirmed by one unfiltered run, 2 files) + **1011** browser (1000 + 11 new `coding-lists.test.ts`, decision 0446 — search/pagination wiring plus picker-independence coverage; 3 pre-existing tests in that same file updated for the new async form-opening behaviour, and the old client-side-indent test replaced), **all green** — confirmed by one unfiltered whole-suite run (48 files). **A second, unrelated pre-existing gap found and fixed along the way**: `test-browser/tasks.test.ts`'s own copy of the "list every nav item" tests never had `nav.apsetup`/"AP Setup" added, the same class of gap decision 0441 already fixed once in `rules.test.ts`'s sibling copy but missed here — confirmed pre-existing via `git stash` before fixing, unrelated to decision 0446 itself. With both fixed, the previously-documented `test-browser/document-window.test.ts` unhandled-rejection flake (160 non-fatal logged errors, confirmed unchanged from baseline) no longer causes any failing assertion anywhere in the browser suite · shared 295 (+3 known pre-existing failures) · `apply_migrations.py --replay-only` clean for the `vf-app` chain (76 migrations/170 invariants, unchanged this decision — no new vf-app migration); `vf-licence`'s own 149-migration chain has no equivalent Python replay, validated instead through `test/setup.ts` + `string-coverage.test.ts` (both run above, both green). **Decision 0447 (unchanged counts across the board — reworded existing assertions, added no new tests): `vf-app` 2618/2618, `vf-licence` 320/320 (migration 0150 adds 6 rows to `ui_strings`, no new test file), `vf-ui` Worker 74/74, browser 1011/1011, all confirmed by unfiltered whole-suite runs; the pre-existing `document-window.test.ts`/`documents.test.ts` unhandled-rejection flake (160 non-fatal errors) reconfirmed identical to baseline via `git stash`.** **Decision 0448: `vf-app` `test/documents.test.ts` 66/66 and `test/ap-assistant.test.ts` 68/68 run directly — the full, whole-repo `vf-app` suite (113 files, ~2618 tests) could not complete inside this session's own tool timeout, and was not re-run in full this decision; decision 0447 already confirmed it as a clean 2618/2618 baseline with no `vf-app` file touched, and this decision's own two files were run individually instead. `vf-licence` 320/320 (unchanged — no new migration, no new string key). `vf-ui` Worker 74/74 (unchanged), browser **1022/1022** (1011 + 11 new in `documents.test.ts`), confirmed by an unfiltered whole-suite run (48 files); the pre-existing unhandled-rejection flake is present at its identical baseline count (160 errors), 6 of them from `documents.test.ts` itself, none a failing assertion.** **Decision 0449: `vf-app`'s full whole-repo suite again could not complete inside this session's own tool timeout (the same limitation decision 0448 already documented); every file plausibly touching the changed code was run individually instead, all green — `test/task-list-route.test.ts` 67/67 (45 pre-existing + 22 new), `test/scoped-roles.test.ts` 48/48 (46 pre-existing + 2 new, covering the search/visibility composition risk — a search term must not widen past what permission-scoping or org-focus already narrowed to — directly), `test/dashboard.test.ts` 64/64 (backward compatibility for `dashboard-route.ts`'s two internal `handleListMyTasks` callers), `test/documents.test.ts` 66/66 and `test/ap-assistant.test.ts` 68/68 (unchanged, re-confirmed), `test/session-routes.test.ts` 15/15 (the one file exercising `/tasks` at the full HTTP-route level via `index.ts`) — 308 tests total across these six files, no regressions. This same-decision discovery is worth naming on its own: a pre-existing regression test (not a new one written for 0449) caught a genuine SQL three-valued-logic bug in the ownership-count query, the strongest evidence yet that this suite is doing real work, not just accumulating. `vf-licence` 320/320 (unchanged — migration `0151` adds rows to the existing `ui_strings` table, not a new test file), confirmed by one unfiltered whole-suite run. `vf-ui` Worker 74/74 (unchanged), browser **1034/1034** (1022 + 12 new in `tasks.test.ts`), confirmed by an unfiltered whole-suite run (48 files); the pre-existing unhandled-rejection flake is present at its identical baseline count (160 errors), none a failing assertion — reconfirmed unchanged via a direct `git stash` comparison.** **Decision 0451: `vf-app`'s full whole-repo suite again could not complete inside this session's own tool timeout; every file anywhere in the codebase importing `INVOICE_FIELDS`/`INVOICE_LINE_FIELDS`/`INVOICE_FIELD_TYPES`/`FIELD_DESCRIPTIONS` was found (ten files) and run directly instead — `shared/interpreter/field-coverage.test.ts` 4/4, `shared/interpreter/vocabulary.test.ts` + `shared/standards/code-lists.test.ts` 37/37 together, `workers/vf-app/test/field-visibility.test.ts` + `test/key-fields.test.ts` **76/76** (70 pre-existing + 6 new, proving the new fields are keyable per line with no route change), `test/ar-process.test.ts` + `test/expense-process.test.ts` 9/9, `workers/vf-ui/test-browser/compose.test.ts` 32/32. `vf-licence` full suite **320/320** (unchanged in count — migration `0152` adds rows to the existing `ui_strings` table, not a new test file), including `string-coverage.test.ts` **10/10**, confirmed by one unfiltered whole-suite run. `shared`'s pre-existing 3 Web Crypto failures in `licensing/token.test.ts` reconfirmed identical on `origin/main` via `git stash`, unrelated to this decision.** **Decision 0452 (Phase 2 of the Cost-Object Approval Hierarchy — the resolver generalization, `workflow-engine.ts`'s multi-task-per-line spawning, the schema, the config route, and both real screens from decision 0450's mock-up): `vf-app` targeted run — `test/approval-hierarchy.test.ts` **30/30** (21 pre-existing + 9 new, proving `resolveApprovalTargets`/`resolveCostObjects` directly: single-dimension backward compatibility, a dimension skipped when uncoded or disabled, two coded-and-enabled dimensions resolving independently — not first-match-wins, chain escalation via `parent_entry_id`, and the no-applicable-dimension fallback matching the old singular behaviour exactly), `test/workflow-engine.test.ts` **53/53** (51 pre-existing + 2 new end-to-end tests — a line coded to two enabled dimensions raises two real tasks, gated by the existing `stage_visit_id`-scoped completion count with no change needed, and an all-or-nothing 409 when one of two coded dimensions can't resolve), `test/coding-list-route.test.ts` + `test/coding-list-csv-route.test.ts` + `test/accounting-frame.test.ts` + `test/approval-config-route.test.ts` run together with the two files above — **213/213** across all six files in one run, confirmed by re-running `approval-hierarchy.test.ts` and `workflow-engine.test.ts` individually too. `migrations/apply_migrations.py --replay-only` clean — **77 migrations, 174 invariants** (up from 76/170 at decision 0446's own report — now includes `0076` Account Coding and `0077` this decision's own Cost-Object schema, neither yet confirmed applied to the live remote). `vf-licence` full suite **320/320** (unchanged in count — migration `0153`'s own Cost-Object Priority panel strings add rows to the existing `ui_strings` table, not a new test file), including `string-coverage.test.ts` **10/10** with the new `apsetup.costobjectpriority`/`.costobjectprioritysub`/`.costobjectenable`/`.costobjectsavefailed` keys added to `KEYS_THE_INTERFACE_USES`, confirmed by one unfiltered whole-suite run. `vf-ui` Worker 74/74 (unchanged — no route or proxy file touched), browser **1040/1040** (1034 + 6 new — 1 in `coding-lists.test.ts` proving the new Approval Limit field always sends alongside the approver in one request, 5 in `ap-setup.test.ts` for the new Cost-Object Priority panel: hidden outside Cost-Object mode, lists all four dimensions in sequence order with their own enabled state, a checkbox toggle saves the full four-row array with only that one flipped, drag-to-reorder — mirroring `processes.js`'s own stage-sequence drag control (decision 0352) rather than the mock-up's own bespoke buttons — sends the whole new order re-sequenced from 0, and a failed save shows the panel's own error), confirmed by an unfiltered whole-suite run (48 files); the pre-existing `document-window.test.ts` unhandled-rejection flake is present at its identical baseline count (160 errors), none a failing assertion. `vf-app`'s full whole-repo suite again could not complete inside this session's own tool timeout (the same limitation decisions 0448/0449/0451 already documented); the targeted run above covers every file this decision's own production code touches.** **Decision 0453 (the invoice-line Coding pop-out — a new `requireAnyPermission` in `enforce.ts`, `Admin.Configure` OR `AP.Validate` now gating `GET /org/cost-centres` and `GET /coding-lists/:type`, and a new `filters` parameter on both routes' own list handlers, all read by `viewer.js`'s new per-line pop-out): `vf-app` targeted run — `test/enforce.test.ts` **18/18** (5 new, `requireAnyPermission` directly), `test/coding-list-route.test.ts` (5 new, the `filters` param narrowing reads), `test/accounting-frame.test.ts` (3 new, the same for Cost Centre's own table), `test/index.test.ts` (10 new, through the real router — both permissions, the write side's own gate unchanged, and `filter.company_code=` proven end to end), run together with `test/cost-centre-route.test.ts`, `test/coding-list-csv-route.test.ts`, `test/key-fields.test.ts`, and `test/field-visibility.test.ts` — **342/342** across all eight files in one run. `vf-licence` full suite **320/320** (unchanged in count — migration `0154`'s own seven new pop-out strings add rows to the existing `ui_strings` table, not a new test file), including `string-coverage.test.ts` **10/10**, confirmed by one unfiltered whole-suite run. `vf-ui` Worker 74/74 (unchanged — no proxy/route file touched), browser **1048/1048** (1040 + 8 new, a new "the invoice-line Coding pop-out" describe block in `viewer.test.ts` — the icon shown regardless of edit permission, Company Code read-only from `stored.buyer.entityName`, an already-keyed Cost Centre resolved to its own name, a field with no stage configuration shown read-only with a note, a field configured `read` shown read-only with none, searching Project through to the page's own Save payload, clearing a chosen value removing it from that payload, and General Ledger Code's own picker proven linked — filtered by Company Code immediately and by Commodity Code only once one is chosen), confirmed by an unfiltered whole-suite run (48 files); the pre-existing `document-window.test.ts` unhandled-rejection flake is present at its identical baseline count (160 errors), none a failing assertion. `vf-app`'s full whole-repo suite again could not complete inside this session's own tool timeout (the same limitation decisions 0448/0449/0451/0452 already documented); the targeted eight-file run above covers every file this decision's own production code touches.** **Decision 0454 (`onTaskCompleted` reports where its cascade stopped instead of silently parking; `index.ts`'s new `followUpAfterTaskCompletion` loads the invoice's real facts and lines and finishes the job with a genuine `visitCurrentStage` call, recording `workflow.stageError` on a real refusal the same way decision 0435 already does for the intake path): `vf-app` — `test/workflow-engine.test.ts` (one existing test extended to assert the new `needsEvaluationAt` return value), `test/index.test.ts` (5 new — an empty rule set correctly fallen through to completion, a rule set that genuinely fires gets a real task rather than a silent stop, a line-scope follow-up correctly loads real stored line facts and fires on them, a genuine `requires_org` refusal recorded as `workflow.stageError`, and a non-invoice subject left exactly as parked as before), run together with `test/task-route.test.ts`, `test/invoice-facts-route.test.ts`, `test/approval-hierarchy.test.ts`, `test/source-capture-workflow.test.ts`, `test/invoice-org.test.ts`, and `test/intake-capture.test.ts` — **288/288**, plus a second pass over every other file importing `invoice-facts-route.ts`/`workflow-engine.ts` (`test/ar-process.test.ts`, `test/compile-route.test.ts`, `test/dashboard.test.ts`, `test/documents.test.ts`, `test/expense-process.test.ts`, `test/fraud-duplicates.test.ts`, `test/invoice-history.test.ts`, `test/key-fields.test.ts`, `test/load-suppliers.test.ts`, `test/stage-permissions.test.ts`, `test/unit-config.test.ts`) — **419/419**, **707/707** total, no regressions. No `vf-ui`/`vf-licence` change, no new migration. `vf-app`'s full whole-repo suite again could not complete inside this session's own tool timeout (the same limitation decisions 0448/0449/0451/0452/0453 already documented); the targeted runs above cover every file this decision's own production code touches.** **Decision 0455 (`index.ts`'s `GET /org/cost-centres` and `GET /coding-lists/:type` widened once more, from `Admin.Configure OR AP.Validate` to also accept `AP.Code`): `vf-app` — `test/index.test.ts` in full (2 new tests confirming `AP.Code` alone now works on both routes, 1 new test confirming the write side, `POST /coding-lists/:type`, still refuses `AP.Code` alone), run together with `test/coding-list-route.test.ts`, `test/cost-centre-route.test.ts`, `test/stage-permissions.test.ts`, and `test/field-visibility.test.ts` — **226/226**, zero regressions. No `vf-ui`/`vf-licence` change, no new migration. `npx tsc --noEmit` surfaces no new errors in `index.ts` or `test/index.test.ts`.** **Decision 0456 (six more routes on the Coding task path widened to also accept `AP.Code` — `GET /invoices/:id` and its document/pages routes, `/progress`, and `POST /invoices/:id/key`): `vf-app` — `test/index.test.ts` in full (9 new tests: six routes each proven to now accept `AP.Code` alone, plus two regression checks that a user holding neither permission is still refused), run together with `test/invoice-facts-route.test.ts`, `test/task-route.test.ts`, `test/session-routes.test.ts`, `test/coding-list-route.test.ts`, `test/cost-centre-route.test.ts`, `test/field-visibility.test.ts`, and `test/stage-permissions.test.ts` — **296/296**, zero regressions. `npx tsc --noEmit` on `index.ts` itself: zero errors before and after, confirmed via `git stash` comparison. No `vf-ui`/`vf-licence` change, no new migration.** **Decision 0457 (Account Coding Suggestions, Phase 1 — the frequency-based default computed from `keyed_fields`, gated the same as the routes beside it): `vf-app` — new `test/coding-suggestions.test.ts` **9/9** (no history, below `MIN_SAMPLE_SIZE`, a clear majority, a too-split history offering nothing, per-supplier isolation, per-field independence, a 404 for a nonexistent invoice, and an empty result for an invoice with no supplier), `test/index.test.ts` 3 new tests through the real router (401, 403, and a full end-to-end pass). This session's own tool timeout finally allowed a complete, unfiltered whole-repo run — **2733/2733 across 114 files**, the first full-suite confirmation since decision 0447; every count from 0448 through 0456 above was either arithmetic or a targeted-file run only. `vf-licence` — new migration `0155` replay-tested clean (155 migrations, all assertions held); `test/string-coverage.test.ts` initially failed with the new key reported as used but nowhere defined — not a wrong key, but `test/setup.ts`'s own hand-maintained migration-import list having never been wired up past `0154`, fixed by adding the missing import and exec call for `0155`. Full whole-repo suite run unfiltered afterward — **320/320 across 21 files**. `vf-ui` — the pre-existing decision-0453 "invoice-line Coding pop-out" describe block (8 tests) re-run first and confirmed still green once `openLineCodingPopout`'s new async suggestion fetch was stubbed; 4 new tests added (pre-fill with the visible note, `Save` persisting a pre-filled suggestion with no explicit click, choosing any value clearing the note, an existing value never overwritten or flagged). Full whole-repo browser suite run unfiltered afterward — **1052/1052 across 48 files** (1048 + 4 new); the pre-existing `document-window.test.ts` unhandled-rejection flake reconfirmed at its identical baseline count (160 non-fatal errors, none a failing assertion) via a direct `git stash` comparison, unrelated to this decision. `npx tsc --noEmit`: zero errors in `coding-suggestions.ts` or the `index.ts` route wiring — the single repo-wide `tsconfig.json` means the same pre-existing, unrelated error count (753 lines, same `cloudflare:test` cross-project resolution gap every test file in the monorepo already has) shows regardless of which workspace directory `tsc` is run from. *(This row originally read 1060/1060 — corrected to the true 1052/1052 while verifying decision 0458's own baseline; a transcription error, not a real gap in what shipped. See decision 0457's own doc.)*** **Decision 0458 (the Coding pop-out is a fixed size, with one shared results area instead of one per field — `.popout.codingpopout`, `codingResultsController`'s generation-token guard against a stale field's slow search overwriting a faster one from wherever the person moved to, and a 2-column unwrapped results grid): `vf-ui` — the pre-existing "invoice-line Coding pop-out" describe block (12 tests) re-run first, unmodified and still green; 4 new tests added (the fixed-size class and DOM structure, exactly one shared results list, the label and contents updating correctly across fields, and the race itself — a manually gated `fetch` mock proving a slower field's stale answer cannot clobber a faster one). Full whole-repo browser suite run unfiltered — **1056/1056 across 48 files** (1052 + 4 new), same pre-existing flake at its identical baseline. `vf-licence` — new migration `0156` replay-tested clean (156 migrations, all assertions held); `test/setup.ts` wired up for it in the same pass. Full whole-repo suite run unfiltered — **320/320 across 21 files** (unchanged in count). No `vf-app` change — `searchableEntryPicker`/`codingResultsController` are only ever called from `openLineCodingPopout`, confirmed by search. `npx tsc --noEmit`: same repo-wide 753-line baseline, unsurprising since this decision touches no TypeScript.** **Decision 0459 (auto-focus and pre-load Cost Centre, the results area back to one column, and a "Narrowed by:" note on a scoped field's own empty result): `vf-ui` — the pre-existing decision-0453/0457/0458 describe blocks (16 tests) re-run first, unmodified and still green; 5 new tests added (Cost Centre holds real focus on open, its own first page preloads with nothing typed, no other field is preloaded alongside it, a plain unscoped empty result stays exactly "Nothing on file matches that.", and General Ledger Code's own scoped empty result names "Org / Company Code"). Full whole-repo browser suite run unfiltered — **1061/1061 across 48 files** (1056 + 5 new), same pre-existing flake at its identical baseline, reconfirmed via `git stash` on this one file in isolation. `vf-licence` — new migration `0157` replay-tested clean (157 migrations, all assertions held); `test/setup.ts` wired up for it in the same pass. Full whole-repo suite run unfiltered — **320/320 across 21 files** (unchanged in count). No `vf-app` change — the General Ledger Code finding is about existing server logic and live data, not a code defect. `npx tsc --noEmit`: same repo-wide 753-line baseline.** **Decision 0460 (every field shows its own first page on `focus`, no minimum length before typing filters results, narrower search boxes, and the Org / Company Code box matched to them): `vf-ui` — the pre-existing decision-0453/0457/0458/0459 describe blocks (21 tests) re-run first, unmodified and still green; 5 new tests added (focusing a field other than the one auto-focused on open still shows its own first page, a single typed character already updates results, clearing a box re-shows its first page rather than blanking it, the Org / Company Code box carries the matching class, and the one race this decision reopened — a slow, gated `resolveCurrent` lookup for an already-keyed field does not fire late and steal the results area from wherever the person has since moved on, closed with the results controller's new `peek()`). Full whole-repo browser suite run unfiltered — **1066/1066 across 48 files** (1061 + 5 new), same pre-existing flake at its identical baseline. No `vf-app`/`vf-licence` change, no new migration — every string reused already existed. `npx tsc --noEmit`: same repo-wide 753-line baseline.** **Decision 0463 (a new `POSSIBLE_DUPLICATE_THRESHOLD = 0.4` constant in `vf-app`'s own `invoice-history.ts`, replacing three files' own separately-stated `0.5`): every `vf-app` test file that reads, seeds, or asserts on `duplicate_confidence` anywhere in the suite, run together — `test/fraud-duplicates.test.ts`, `test/dashboard.test.ts`, `test/documents.test.ts`, `test/invoice-facts-route.test.ts`, `test/invoice-history.test.ts`, `test/ap-assistant.test.ts`, `test/index.test.ts` — **395/395**, zero regressions; 1 new test proves the live report directly (two invoices submitted through the real `handleUpsertInvoice()`, same supplier/total/date, only the invoice number incremented, now shows up), and the two pre-existing boundary tests now assert against the constant itself rather than a restated literal. No `vf-ui`/`vf-licence` change, no new migration, no new string. `npx tsc --noEmit`: **755 lines**, two more than the prior 753-line baseline — confirmed via direct `git stash` comparison to be one new, same-pattern `TS2352` cast (every other test in this same file already has one) on the one new test, not a new class of error.** **Decision 0469 (Phase 1 of Matching Exceptions and Non-PO Approval routing — the four split `po.line_*` vocabulary facts, `po-matching.ts`'s org-wide default tolerance and `quantity_matching_enabled` toggle, `Procurement.Collaborate`/`Procurement.Approve`, and Non-PO Approval routing itself across `approval-hierarchy.ts`/`workflow-engine.ts`): `shared/interpreter/vocabulary.test.ts` 23/23. `vf-app` — `test/po-matching.test.ts` **23/23** (16 pre-existing + 7 new), `test/approval-hierarchy.test.ts` **36/36** (30 pre-existing + 6 new), `test/workflow-engine.test.ts` **56/56** (53 pre-existing + 3 new end-to-end), `test/stage-permissions.test.ts` + `test/org-route.test.ts` run together **142/142**, `test/index.test.ts` **152/152**. This session's own tool timeout allowed a complete, unfiltered whole-repo run, run twice for confirmation — **2750/2750 across 114 files**, both times (2741 + 9 new). No `vf-ui`/`vf-licence` change — nothing in either references the new vocabulary, permissions, or routing yet, confirmed by search; no new migration needed there either. `migrations/apply_migrations.py --replay-only` clean — 78 migrations, 184 invariants (up from 77/174 at decision 0452's own report — now includes `0078` this decision's own schema, not yet confirmed applied to the live remote). `python3 scripts/check-citations.py` clean — 469 records, none dangling. `npx eslint` clean on every file this decision touches (two pre-existing, out-of-diff findings in `test/workflow-engine.test.ts`, confirmed unrelated to this decision's own diff via `git diff`). `npx tsc --noEmit`: no new error class — the same repo-wide `cloudflare:test` cross-project resolution gap every test file in the monorepo already has, plus the same two pre-existing, out-of-diff lines `eslint` also flagged.** **Decision 0470 ("Add person to conversation" — Phase 2, `invoice-collaborators-route.ts`'s `handleAddCollaborator`/`handleListCollaborators`/`isInvoiceCollaborator`, five existing routes widened via a new `canViewInvoiceAsCollaborator` helper — the first per-record access check this codebase has needed — plus two new routes and `org-route.ts`'s new `handleSearchUsers`, and `vf-ui`'s new `collaborators.js`): `vf-app` — new `test/invoice-collaborators-route.test.ts` **11/11**, `test/org-route.test.ts` **142/142** (136 pre-existing + 6 new for `handleSearchUsers`), `test/index.test.ts` **161/161** (152 pre-existing + 9 new through the real router, proving both the permission and the per-invoice listing are independently required and that one invoice's roster does not leak access to another), run together with `test/activity-route.test.ts`, `test/stage-permissions.test.ts`, and `test/team-route.test.ts` beforehand — **350/350**, zero regressions. `vf-licence` full suite **320/320** (unchanged in count — migration `0158` adds rows to the existing `ui_strings` table, not a new test file), including `string-coverage.test.ts` **10/10** with the five new `activity.addperson*`/`activity.collaborators` keys required. `python3 scripts/check-citations.py` clean — 470 records, none dangling. `npx eslint` clean on every touched file, backend and frontend; `node --check` clean on both new/edited frontend JS files. `npx tsc --noEmit`: no new error in any touched file — the same repo-wide `cloudflare:test` noise plus pre-existing, out-of-diff findings in files this decision did not touch. **A full, unfiltered `vf-app` suite run did not complete inside this session's own tool timeout** — the same limitation decisions 0448/0449/0451/0452/0453/0454 already documented; verification here is the targeted run above, the same discipline those decisions used for the same reason. No `vf-app` migration this time — `invoice_collaborators`/`Procurement.Collaborate` both already existed and were already live from decision 0469.** |
| Decision records | 470 |

**Decision 0479 (`.c-process` shares its stretched height, instead of
every panel claiming it whole) is built, tested, documented, and
confirmed pushed and deployed** — `origin/main` fetched directly reads
`f1a81b1`, matching this session's own commit exactly, and the operator
confirmed with *"pushed and deployed successfully."* Reported live, cautiously,
right after decision 0478 had already been confirmed working: *"I've
noticed a page orientation issue. which might be new, but not sure."*
Two screenshots and a precise description followed: the "Here because"
and "Document open in a separate window" cards at the top too tall and
misaligned, the process timeline seeming to overlay the Seller/Buyer
cards beneath it, and the Invoice Header card not as tall as
Seller/Buyer. **A real bug, and decision 0478's own doing, not user
error.** `#viewer .columns.docpoppedout .c-process > .panel { height:
100%; }` (decisions 0392/0393) was written for exactly one panel;
0478's own `reasonLinePanel()` made `.c-process` commonly hold two or
three panels at once (`workflowErrorPanel()`, `reasonLinePanel()`,
`progressRow()`), and the old rule gave *each one* the row's full
height — stacked, overflowing the row by up to 200–300%, which is
exactly what read as the timeline "overlaying" the row beneath it.
**Fixed with flex, not a second height rule**: `.c-process` becomes a
flex column; banner panels keep their own natural height at `flex: 0 0
auto`; only the last panel — always `progressRow()` when present —
grows to fill the rest at `flex: 1 1 auto; min-height: 0`. With one
panel this reproduces the old behaviour exactly, so the ordinary,
single-panel case is unchanged. **A related, independently
pre-existing gap found in the same screenshot**: decision 0393's own
`height: 100%` fix for `.c-parties` only ever covered
Header-taller-than-Parties, the one direction observed at the time. A
Seller card long enough to need it (an unmatched-supplier notice plus
several fields) made Parties the taller side for the first time —
fixed with the symmetric rule on `.c-header > .panel`. Verified
visually, not just argued about: a standalone Playwright screenshot
harness (scratch-only, not part of the repo) reproduced the exact bug
on the already-pushed CSS (`c48882f`) and confirmed it gone on the fix,
across the one-, two-, and three-panel cases. Verification: `vf-ui`
browser suite **1092/1093** (the one failure pre-existing and
unrelated — the same `typography.test.ts` hardcoded `10px` finding
decision 0478 already documented), 2 new tests in `viewer.test.ts`
asserting the actual CSS rule text (jsdom computes no real layout).
`vf-ui` Worker unchanged at 74/74. `eslint public test-browser` has one
pre-existing, unrelated error (`ap-setup.test.ts:588`, unused `puts`),
confirmed identical on unmodified `main`. No `vf-app`/`vf-licence`
change, no new migration. Full reasoning in decision 0479.**

**Decision 0478 ("Here because" — why a task landed on you) is built,
tested, documented, and confirmed pushed and deployed** — `origin/main`
fetched directly reads `c48882f`, matching this session's own commit
exactly. The operator first reported *"I don't see the change in the
UI yet"* after deploying and applying migrations; the cause was the
decision's own deliberate non-retroactivity (a task already open before
the deploy has `rule_id = NULL` and nothing to show a reason for) —
explained, and confirmed correct once the operator cleared runtime data
and resubmitted the same test invoices: *"confirmed - I cleared out
data again and resubmitted the previous invoices. I see the message
now."* Reported live against a real `assign_task` permission bug
(`requiredPermission "undefined"`, root-caused and fixed by the
operator re-authoring their own rule to state its permission
explicitly — no code change), then broadened into a product question:
*"for a user who retrieves that task, it is difficult to initially
tell why it has been routed to that stage... Something like 'This item
has been routed to you because of a business rule being triggered -
<Rule Name>.'"* An interactive mockup of two placements was shown
directly before any code was written; the operator chose Option A, a
slim line above the stage progress bar, deliberately kept structurally
separate from `workflowErrorPanel` (decision 0435) — that names an
engine failure, this names an ordinary, working rule outcome, and
conflating them would make routine routing look like a fault. A rule's
own *name* is translatable (the four standard matching rules through
the ordinary `ui_strings` table by their stable `key`; a customer's own
authored rule name through a new `rule_name_translations` table) but
its full *sentence* is never translated — shown on request exactly as
authored, in whatever language that was. Building it surfaced a real,
pre-existing gap: `evaluateRuleSet` has always paired a fired action
with the rule that fired it, and `workflow-engine.ts`'s own
`assign_task` handling discarded that rule id before a task was even
created — no task anywhere had ever recorded which rule raised it,
fixed here via a new nullable `tasks.rule_id` (migration `0079`).
`GET /invoices/:id` now takes a per-request `?locale=`, unlike every
other backend locale use in `vf-app`, which is per-deployment.
`rule.js` gained a German-only translation control (the only locale
the language picker actually exposes, decision 0302). Verification:
`vf-app` — `workflow-engine.test.ts` **60/60**, `rules-list.test.ts`
**43/43**, `key-fields.test.ts` **56/56**, plus 934 further tests
across every file touching `tasks`/`rules`/task routes, all green
except one pre-existing, unrelated `stage-permissions.test.ts` failure
confirmed present on unmodified `origin/main`. `vf-licence` migration
chain (163 migrations) replays clean. `vf-ui` browser suite
**1090/1091** (the one failure pre-existing and unrelated —
`typography.test.ts`'s hardcoded `10px` finding, nothing to do with
this decision), `rule.test.ts` **26/26**, `viewer.test.ts` **186/186**,
all new. Full reasoning in decision 0478.**
...
**Decision 0444 (Account Coding: Company code, Cost Centre, Project,
Commodity Code, and General Ledger Code) is built and tested. Not yet
pushed or deployed.** Investigating Cost-Object mode's own missing UI
turned into a much larger ask: *"Cost-Center Lists should be
maintained under the Account Coding tab, with other valid coding
lists. This would include Company code (Org), Cost-Center; Project,
Commodity Code, General Ledger Code for example."* Five example CSV
exports followed, all sharing one shape (`ID, Path, Default, Approver,
Parent List ID, Parent Entry ID`, plus dynamic "Filter by - X"
columns) — confirmed with the operator directly: one generic framework
rather than five bespoke tables, real parent pointers for Project's own
hierarchy (the source export's own dot-notation convention, `Parent
Entry ID` blank throughout), and manageable lists only for the three
genuinely greenfield ones (Project, Commodity Code, General Ledger
Code), the same precedent decision 0031 already set for Cost Centre.
Grounded in real SAP/Oracle practice at the operator's own request,
checked directly rather than assumed: Oracle's own Accounting
Flexfield segments (Company/Cost Center/Account/Project) are close to
a one-to-one match for these five lists; Project's hierarchy is
structurally SAP's own WBS element numbering; a GL Code's stored
default Commodity Code is the same shape as SAP's own automatic
account determination (OBYC), stored as reference data only per the
confirmed scope. New: `migrations/0076_account_coding_lists.sql`
(`coding_list_types`, `coding_list_type_filters`, `coding_list_entries`,
`coding_list_entry_filters`) and `coding-list-route.ts` (vf-app);
`coding-lists.js` (vf-ui) for the tab's own real content, replacing its
placeholder. Company code stays read-only (`org_units`, already
managed via Access); Cost Centre keeps its own existing table and
routes entirely unchanged, gaining a real management screen (none
existed before) and a new "Filter by company code," stored the same
generic way rather than a new rigid column — 0016's own header comment
is explicit a cost centre is deliberately not foreign-keyed to
`org_units`. **The same proxy-allowlist gap decisions 0212, 0319,
0324–0328, 0415, 0417–0430, and 0441 already found repeatedly**: two
existing, real, tested `vf-app` routes (`POST /org/cost-centres`,
`PUT /cost-centres/:id`) had never been added to `vf-ui`'s own
allowlist either — found and fixed here rather than left for an
eighteenth report of the identical symptom. `vf-app` +27 tests
(`coding-list-route.test.ts` new, `accounting-frame.test.ts`
extended), full unfiltered run green; `vf-licence` +0 test files
(migration `0147`, 22 keys × en/de, added to `string-coverage.test.ts`'s
own coverage list — which was found, while adding these, to have never
covered any `apsetup.*` key at all, across decisions 0440, 0442, and
0443 either; named, not backfilled), full run green; `vf-ui` +11
browser tests (`coding-lists.test.ts` new) and one stale test in
`ap-setup.test.ts` corrected (Account Coding is no longer a
placeholder), full unfiltered runs both suites green; `eslint` clean
across all three workspaces. **Deliberately not built**: any delete
route anywhere in this decision (matching Cost Centre's own existing
create-and-edit-only precedent); a ledger management screen (none
exists yet at all — a cost centre's parent still works with no ledger
set, since the same-ledger check passes when both sides are `NULL`,
but assigning a *different* ledger has nowhere to do it yet); SAP's own
multi-company-code cost-centre sharing (this decision's "Filter by
company code" is single-valued, matching the operator's own example
data exactly); actually deriving a GL code from a commodity code
during invoice coding (the data now exists; the derivation does not).
See decision 0444 for the full reasoning, the complete grounding in
SAP and Oracle practice, and the full test list.

**Decision 0443 (the Add button becomes an icon button, top-right of
its own card) is built, tested, and confirmed pushed and deployed** —
`origin/main` fetched directly reads `9655bfe`, matching this session's
own commit exactly, and the operator confirmed with *"pushed and
deployed - thank you."* Asked directly, from a screenshot, once decision 0442 deployed: *"create a
suitable icon for the Add button and move to the top-right of each
card."* Not a new pattern — `modeForm()`, one form above these two
sections on the same tab, already renders its own Save button as an
`actionLink` inside `.cardhead`'s own `.statebuttons` slot, the same
title-left/action-right shape `.cardhead`'s own CSS already gives
(`display: flex; justify-content: space-between`) and every other
screen's Create/Save button already uses. The plain `<button>Add</
button>` and its own `.memberpickerrow` wrapper, in both override
sections, are replaced with `actionLink("create", {primary: true,
label: t("apsetup.add"), onclick})` inside that section's own
`cardhead` — the `create` icon this app already draws for "add a new
record" everywhere else, not a new one; the label stays "Add," not
"Create," via the same `label` override `sources.js`'s own "Load CSV"/
"CSV Template" buttons already use. No new strings, no new icon, no
CSS changes. **The same screenshot also showed the new search-hint
text as raw, untranslated keys** — not a code bug: both keys match
`ap-setup.js`'s own calls and migration `0146`'s own rows exactly, and
`t()` returns the raw key when a string is genuinely missing from what
loaded. Read as the same shape decision 0441's own second finding
already was, rather than re-traced in code — confirmed correct once
the operator applied `0146` for real: *"yes, I see the proper text
hints now."* New: one
test per override section confirming the Add button now lives at
`.cardhead .statebuttons .actionlink`, carries a real `<svg>`, still
reads "Add," and the old `.memberpickerrow` is gone (`ap-setup.test.ts`
18 → **19**). `vf-ui` browser suite 978 → **979**, 975 green, 4
failed — the same already-documented, pre-existing `document-window.
test.ts` flake, unrelated. `vf-ui` Worker suite, `vf-app`, `vf-licence`
all untouched — no route, schema, or string change. `eslint .` clean.
See decision 0443 for the full reasoning and tests.

**Decision 0442 (the override lists move below their own add-row forms,
and gain search) is built, tested, and confirmed pushed and deployed**
— the operator confirmed with *"deployed and pushed."* **Its own
migration `0146` is now also applied and confirmed live** — *"yes, I
see the proper text hints now."* Raised
directly by the operator, once decision 0441 closed out the
loading bug: *"These overrides could get large. Does the UI cap the
height of the list, and leverage scrolling?"* Checked before answering
— no, `ap-setup.js`'s two override lists reuse the same
`.assignmentlist` component `access.js`'s own team-member and role
lists already use, and nothing anywhere caps or scrolls it; not a
defect 0440 introduced, the same unbounded shape already exists on
those sibling screens. The operator's own follow-up named a concrete
fix: *"I think it would make sense for the list to build below the
configuration boxes... Perhaps also make the list searchable and
paginated, as the Document search looks."* Checked directly rather
than assumed: Documents' own "search" is a query box plus a
server-side-capped result set with a "shown of total" note — this app
has no real page-number pagination anywhere. Two forks put to the
operator before building, both answered: match Documents' own
capped-list-plus-search shape (not invent real pagination fresh), and
filter **client-side** (`GET /approval-config` already returns every
override in one call, so the whole list is already in the browser).
Built: both override sections now render their add-row form first,
then a new search box, then the (potentially long) list of existing
overrides; a shared `searchableOverrideList()` helper filters by a
case-insensitive substring match (person/org/supervisor for one list,
person/org/currency/amount for the other), caps display at 50 rows —
the same default Documents' own `limit` falls back to, reused here as
a display cap rather than a fetch cap — and shows a "{shown} of
{total} matching" note when more exist. Four new strings, migration
`0146`, wired into `vf-licence/test/setup.ts` continuing the same
resumed discipline `0145` restarted. Nothing server-side changed —
`vf-ui`-only. New: 5 tests in `ap-setup.test.ts` (13 → 18); full
`vf-ui` browser suite 973 → **978**, 974 green, 4 failed — the same
already-documented `document-window.test.ts` flake decisions 0440 and
0441 both already found, unrelated here; `vf-ui` Worker suite
unaffected at 74; `vf-licence` **320** (row count grew via `0146`, test
count did not); `vf-app` untouched. `eslint .` clean; migration chain
replays clean (146 migrations). See decision 0442 for the full
reasoning and tests.

**Decision 0441 (the proxy never carried its own new routes) is built,
tested, and confirmed pushed and deployed** — `origin/main` fetched
directly reads `3aadf95`, matching this session's own commit exactly;
the operator confirmed with *"deployed and pushed."* Reported live,
immediately after confirming decision 0440 itself pushed and deployed:
*"I do get a message saying 'AP Setup could not be loaded', when I
click on the side menu item - AP Setup."* `vf-ui` is not just static assets — it is
...
named, unrelated to either fix here.

**Deploying this alone did not resolve the original report.** The
operator redeployed `vf-ui` and reported back: *"deployed and pushed -
but I get the same error?"* Same wording, a genuinely different cause
— confirmed via the operator's own browser console: `/api/org/overview`
now returned `200`, `/api/approval-config` now returned `500`, proving
the proxy fix itself had worked (a missing allowlist entry gives `404`,
not `500`). Traced to `handleGetApprovalConfig`'s own first query
against `org_approval_config` — decision 0439's own migration `0075`,
reported confirmed applied at the time, but never actually run against
`vf-app-poc` for real. See the `vf-app-poc` migrations row above for
the full finding; fixed with no code change, by the operator running
`apply_migrations.py --remote`, confirmed working with *"that worked -
thank you."* See decision 0441 for the full reasoning and tests.

**Decision 0440 (the AP Setup screen, and the Approval Hierarchy write
API) is built, tested, and confirmed pushed and deployed** —
`origin/main` fetched directly reads `fc62876`, matching this
session's own commit exactly, and the operator confirmed with *"pushed
and deployed."* In direct response to *"yes please - lets go"* — the
operator's own green light to build the piece decision 0439
...
| 0337, 0338, 0339, 0346, 0347, 0441, 0446 | A test file's own strings stub | The new keys a change actually introduced, not the ones the file already had — hit seven separate times across two arcs; 0441 fixed `rules.test.ts`'s own copy of the "list every nav item" test for `nav.apsetup`, and 0446 found the *sibling* copy in `tasks.test.ts` still had the identical gap, missed at the time because the two files were never checked together |
| 0446 (this session's own delivery mechanics, not the codebase) | `git bundle create` | The branch name — `git bundle create file.bundle 2adc8bd..HEAD` records the ref as a bare, anonymous `HEAD`, not `refs/heads/main`, so the operator's own `git pull file.bundle main` failed with "couldn't find remote ref main" even though the bundle itself was valid and complete. **Always bundle with the branch name, not `HEAD`**: `git bundle create file.bundle <base>..main`. `git bundle verify` on the output should show `refs/heads/main`, not bare `HEAD`, before delivering — checked once as a habit from here on, not assumed. |
...
| 0445 | `vf-licence/test/setup.ts`'s own migration-SQL statement splitter (`toOneStatementPerLine`) | Every string value across 147 prior migrations, none of which had ever contained a literal `;` — splits blind to quoted string literals, so decision 0445's own help text (three `;`-joined clauses) broke 267 of 320 tests until reworded |
...
0442/0443), and `0076` (decision 0444, above). Every time: the code and
the worker deploy were genuinely correct, `apply_migrations.py
--remote` was flagged in the delivery note, and it still didn't run as
its own separate step. Every time the live symptom pointed straight at
it once looked at directly — a `500` (not `404`) on exactly the routes
touching the new tables, or a raw untranslated string key — and every
time the fix needed no code change, only the operator running the
command already named. **Flagging the step in a delivery note is not
the same as it running.** Until this project has a real CI gate or a
single combined deploy-and-migrate command, expect this to keep
happening, and diagnose it first, not last, the moment a "could not be
loaded" or a raw-key report follows a delivery that touched schema.
